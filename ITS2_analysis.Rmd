...
```{r}
#try glom use raw counts its2.s2
coralDIV2_rare2_T0<-subset_samples(coralDIV2_rare2, Time.Point=="T0") #use raw counts here

T0_its2 <- coralDIV2_rare2_T0 %>% 
  tax_glom(taxrank = "DIV", NArm = FALSE) %>% 
  transform_sample_counts(function(x) {x/sum(x)} ) 
...
T0.p <- ggplot(T0_its2.melt, aes(x = Species, y = Abundance, fill = Clade)) +
  geom_bar(stat = "identity") +
  #scale_fill_manual(values=mycolors) +
  ylab("Relative Abundance") +
  xlab("Sample Label") +
