...
# START HERE Analysis REL Abundance -  
```{r}    
library(tidyverse)
library(janitor)
...
coralPro <- coral_profile %>% 
  subset_samples(!is.na(host_species))
coralPro_rel <- transform_sample_counts(coralPro, function(x) 100 * x/sum(x))
...
coralDIV2_rare <- rarefy_even_depth(coralDIV2_nonrare, sample.size = 5000, rngseed = 711)   #this removed 17 DIVs
...
# EDIT sample data
 ...
 #Mcap
  sd$Clade[sd$Parent.ID==363]<-"C31"
  sd$Clade[sd$Parent.ID==364]<-"C31D4"
  sd$Clade[sd$Parent.ID==365]<-"C31D4"
  sd$Clade[sd$Parent.ID==366]<-"C31D4"
  sd$Clade[sd$Parent.ID==367]<-"C31"
  sd$Clade[sd$Parent.ID==368]<-"C31D4"
  sd$Clade[sd$Parent.ID==369]<-"C31D4"
  sd$Clade[sd$Parent.ID==370]<-"C31"
  sd$Clade[sd$Parent.ID==371]<-"C31"
  sd$Clade[sd$Parent.ID==372]<-"C31"
  sd$Clade[sd$Parent.ID==373]<-"C31"
  sd$Clade[sd$Parent.ID==374]<-"C31D4"
 #Pcomp
...
#Pacu
  sd$Clade[sd$Parent.ID==311]<-"C42_2"
  sd$Clade[sd$Parent.ID==301]<-"C1d"
  sd$Clade[sd$Parent.ID==304]<-"C1d"
  sd$Clade[sd$Parent.ID==306]<-"C1d"
  sd$Clade[sd$Parent.ID==303]<-"C1d"
  sd$Clade[sd$Parent.ID==307]<-"C1d"
  sd$Clade[sd$Parent.ID==308]<-"C1d"
  sd$Clade[sd$Parent.ID==310]<-"C1d"
  sd$Clade[sd$Parent.ID==312]<-"C1d"
  sd$Clade[sd$Parent.ID==302]<-"C1d"
  sd$Clade[sd$Parent.ID==305]<-"C42_2"
  sd$Clade[sd$Parent.ID==309]<-"C42_2"
  
sd$Clade<-as.factor(sd$Clade)
sd$Clade <- factor(sd$Clade, levels = c("C31", "C31D4", "C15cj","C15n","C1","C27","C1d","C42_2"))
...
Test just T0 for manuscript
```{r}
#try glom use raw counts its2.s2
profiles_T0<-subset_samples(profiles, Time.Point=="T0") #use raw counts here
...
T0.p <- ggplot(T0_its2.melt, aes(x = Species, y = Abundance, fill = its2_type_profile)) +
  geom_bar(stat = "identity") +
  #scale_fill_manual(values=mycolors) +
  ylab("Relative Abundance") +
  xlab("Sample Label") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1));T0.p
 # theme(legend.position = "none");T0.p
#T0.p +    facet_wrap(~ Species, nrow = 1, scales = "free_x") # Use scales = "free_x" to adjust x-axis per facet
T0.p <- ggplot(T0_its2.melt, aes(x = Species, y = Abundance, fill = its2_type_profile)) +
  geom_bar(stat = "identity") +
  #scale_fill_manual(values=mycolors) +
  ylab("Relative Abundance") +
...
#manuscript figure 2 ####
ellipse_colors <- c("Montipora_capitata" = "#E69F00", "Porites_compressa" = "#56B4E9", "Pavona_varians" = "#CC79A7", "Pocillopora_acuta" = "#009E73")

#point_colors <- c("C" = "goldenrod1", "CD" = "darkorange3", "C15cj" = "powderblue", "C15n" = "#0072B2", "C1"="pink1", "C27"="firebrick", "CCC"="palegreen2", "C42"="darkgreen")  # Replace with actual colors

library(cowplot)  # Make sure to install this package if not already installed


#FIGURE 2B ITS2 T0
# Define your colors
point_colors <- c("#E69F00", "#56B4E9","#CC79A7", "#009E73")  # Adjust these as needed

its2plot<-ggplot(scores.ord.DIV_RelA_stats.T0, aes(x = NMDS1, y = NMDS2, color = Species)) + 
  geom_point(size = 2) +
  geom_mark_ellipse(aes(fill = Species), alpha = 0.2, linetype = 1, size=.5) + # Use geom_mark_ellipse for both fill and border
  scale_color_manual(values = point_colors) +  # Set colors for points
  scale_fill_manual(values = point_colors) +  # Set fill colors for ellipses
    xlim(-2.5, 3) +  # Set lower bound of x-axis to -2.5
    ylim(-2.5, 2.5) +  # Set lower bound of x-axis to -2.5
...
ggsave("Figures/NMDS_ITS2_T0.pdf", plot = its2plot, device = "pdf", width = 4, height = 3)
...
bc.mcap.T0 <- phyloseq::distance(mcap.RelRare.T0, method = "bray") 
    ...
    set.seed(30)
    adonis2(bc.mcap.T0 ~ Clade, data = mcap.DIV.samp.df.T0) #adonis test no clade
...
bc.Pcomp.T0 <- phyloseq::distance(Pcomp.RelRare.T0, method = "bray") 
    ...
    set.seed(30)
    adonis2(bc.Pcomp.T0 ~ Clade, data = Pcomp.DIV.samp.df.T0) #adonis test no clade
