...
#T1 by clade richness
 #C
  erich.tab2.mcap.T1.C<-subset(erich.tab2.mcap.T1, Clade=="C")

  Mcap_alpha_mod<-lmer(observed ~ Treatment+(1|Parent.ID), data=erich.tab2.mcap.T1.C) #adding tank is singularity
  anova(Mcap_alpha_mod) 
  qqPlot(residuals(Mcap_alpha_mod)) 
  emm<-emmeans(Mcap_alpha_mod,  ~ Treatment, adjust="Tukey")
  cld(emm)
  pairs(emm)
  #CD
  erich.tab2.mcap.T1.CD<-subset(erich.tab2.mcap.T1, Clade=="CD")

  Mcap_alpha_mod<-lmer(observed ~ Treatment+(1|Parent.ID), data=erich.tab2.mcap.T1.CD) #adding tank is singularity
  anova(Mcap_alpha_mod) 
  qqPlot(residuals(Mcap_alpha_mod)) 
  emm<-emmeans(Mcap_alpha_mod,  ~ Treatment, adjust="Tukey")
  cld(emm)
...
#by clade
erich.tab2.mcapT1<-subset(erich.tab2.mcap, Time.Point=="T1")
erich.tab2.mcapT1mean <- ddply(erich.tab2.mcapT1, c("Treatment", "Clade"), summarise, #summarize cell counts
                 N    = length(evenness_pielou[!is.na(evenness_pielou)]), #calculate the length of the data frame, excluding NA’s
                 mean = mean(evenness_pielou, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(evenness_pielou, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
erich.tab2.mcapT1mean

Mcap_evenness_pielou_plot<-ggplot(data=erich.tab2.mcapT1mean, aes(x=Clade, y=mean, group = Clade, color=Treatment)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = F)+
  geom_point(aes(color=Treatment), size=4, show.legend = F)+
 # geom_line(aes(color=Treatment), linetype=2, show.legend = F)+
...
#by clade
erich.tab2.mcapTF<-subset(erich.tab2.mcap, Time.Point=="TF")
erich.tab2.mcapTFmean <- ddply(erich.tab2.mcapTF, c("Treatment", "Clade"), summarise, #summarize cell counts
                 N    = length(evenness_pielou[!is.na(evenness_pielou)]), #calculate the length of the data frame, excluding NA’s
                 mean = mean(evenness_pielou, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(evenness_pielou, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
erich.tab2.mcapTFmean

Mcap_evenness_pielou_plot<-ggplot(data=erich.tab2.mcapTFmean, aes(x=Clade, y=mean, group = Clade, color=Treatment)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = F)+
  geom_point(aes(color=Treatment), size=4, show.legend = F)+
 # geom_line(aes(color=Treatment), linetype=2, show.legend = F)+
