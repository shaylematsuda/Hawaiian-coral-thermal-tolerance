...
combined_plot <- ggplot(scores.ord.DIV_RelA_stats.T0, aes(x = NMDS1, y = NMDS2)) + 
  geom_point(aes(colour = Clade), size = 4) +
  stat_ellipse(aes(group = Species, colour = Species), linetype = 2) +
  scale_color_manual(values = c(point_colors, ellipse_colors)) +  # Combine both color scales
  ggtitle("NMDS - ITS2 Profile groups Bray Curtis T0") +
  theme_classic()
