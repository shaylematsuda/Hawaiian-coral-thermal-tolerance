...
Bac.s.rel_Pvar<-subset_samples(Bac.s.rel, Species=="Pocillopora_acuta")
    Bac.s.rel_Pvar = prune_taxa(taxa_sums(Bac.s.rel_Pvar) > 0, Bac.s.rel_Pvar) #this removes any OTU with 0s
...
# With taxonomy:
tax_table(Bac.s.rel)[pairwise_shared[[1]]$shared_taxa, ]
``` 
...
#adonis ########## IF YOU KEEP THIS CHANGE TO ADONIS2 AND REDOSTATS 
set.seed(30)
...
#Permanova UNIFRAC by ASV 20220514 - REDOING adonis T0-T1, T1-TF by TREATMENT (high only, ambient only)
Here, models run within treatment group (high, ambient) with all T0 genotypes represented in both treatments (not split)
Table S11: 
```{r}         
#copy amb T0
...
#Figure S13 T1 by lineage and treatment
