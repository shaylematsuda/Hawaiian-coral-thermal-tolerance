...
Set up, Bac.s rela
```{r}  
#make relativ e abundance df Bac.s2
...
adonis(Bac.s.u.T1F ~ Species*Treatment*Time.Point*Clade, data = s.u.samp.df)
     ...
     # Homogeneity of dispersion test  
    set.seed(30)
    permutest(betadisper(Bac.s.u.T1F, s.u.samp.df$Species, type = "centroid")) #put in distance matrix
   set.seed(30)
    permutest(betadisper(Bac.s.u.T1F, s.u.samp.df$Treatment, type = "centroid")) #put in distance matrix
    
# #dipersion lmer  <-- only if necessary!
#     wu.all<-betadisper(Bac.s.u.T1F, s.u.samp.df$Species, type = "centroid")
# wu.disp<-as.data.frame(wu.all$distances) #extract distances
# names(wu.disp)[names(wu.disp) == "wu.all$distances"] <- "wu.distances" #rename
# wu.disp.Species<-cbind(wu.disp, s.u.samp.df) #bind dist to sd
# hist(wu.disp$wu.distances) #none of these are great
# # hist(log10(wu.disp$wu.distances))
# # hist(sqrt(wu.disp$wu.distances))
# # hist((wu.disp$wu.distances)^1/3)
# # hist(1/wu.disp$wu.distances)
# 
...
adonis(pvar.u.u.T1F ~ Treatment*Time.Point, data = pvar.u.u.samp.df.T1F) #adonis test no clade
  ...
  # Homogeneity of dispersion test  
   ...
   set.seed(30)
    ...
    set.seed(30)
    permutest(betadisper(pvar.u.u.T1F, pvar.u.u.samp.df.T1F$Code, type = "centroid")) #by clade*treatment
...
#T1
 ...
 # Homogeneity of dispersion test  
   ...
   set.seed(30)
    ...
    set.seed(30)
    permutest(betadisper(Pacu.u.T1, pacu.u.samp.df.T1$Code, type = "centroid")) #put in distance matrix
...
#make bray curtis data matrix -T1 and TF
 ...
 #heatmap
   ...
   set.seed(30)
    permutest(betadisper(bc.mcap.g.T1F, mcap.g.samp.df.T1F$Treatment, type = "centroid")) #put in distance matrix
