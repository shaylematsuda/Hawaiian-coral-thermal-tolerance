...
# # export into table 
# taxCN  = tax_table(Bac.s)
# taxCN = as(tax_table(Bac.s), "matrix")
...
# 
# 
# OTUCN = otu_table(Bac.s)
# # Extract abundance matrix from the phyloseq object
# OTU.cn = as(otu_table(OTUCN), "matrix")
# if(taxa_are_rows(Bac.s)){OTU.cn <- t(OTU.cn)}
...
#TF
 ...
 #try plotting only the top 10 FAMs    mcap.u.f.bac.T1F
    #Sort the Phyla by abundance and pick the top 5
    top10P.names.mcap = sort(tapply(taxa_sums(mcap.u.f.bac.T1F), tax_table(mcap.u.f.bac.T1F)[, "Family"], sum), TRUE)[1:10]
...
#TF
 ...
 #try plotting only the top 10 FAMs  ####   
    #Sort the Phyla by abundance and pick the top 5
    top10P.names.Pvar = sort(tapply(taxa_sums(pvar.f.f.bac.T1F), tax_table(pvar.f.f.bac.T1F)[, "Family"], sum), TRUE)[1:10]
      #Cut down the  data to only the top 10 Phyla
      ...
      #rename Fam with taxa
            ...
            names(Pvar.u.f.bac.T1F.top10P.OTU.df)[names(Pvar.u.f.bac.T1F.top10P.OTU.df) == "Otu00079"] <- "uncultured_Otu00079"
            names(Pvar.u.f.bac.T1F.top10P.OTU.df)[names(Pvar.u.f.bac.T1F.top10P.OTU.df) == "Otu00872"] <- "uncultured_Otu00872"
            names(Pvar.u.f.bac.T1F.top10P.OTU.df)[names(Pvar.u.f.bac.T1F.top10P.OTU.df) == "Otu00270"] <- "uncultured_Otu00270"
            names(Pvar.u.f.bac.T1F.top10P.OTU.df)[names(Pvar.u.f.bac.T1F.top10P.OTU.df) == "Otu00533"] <- "uncultured_Otu00533"
            names(Pvar.u.f.bac.T1F.top10P.OTU.df)[names(Pvar.u.f.bac.T1F.top10P.OTU.df) == "Otu00563"] <- "uncultured_Otu00563"
            names(Pvar.u.f.bac.T1F.top10P.OTU.df)[names(Pvar.u.f.bac.T1F.top10P.OTU.df) == "Otu01647"] <- "uncultured_Otu01647"
...
#Means
mcap.16s.fam.T01F.top10.m_mean <- ddply(mcap.16s.fam.T01f.top10.m, c("Treatment", "Clade", "Time.Point","family"), summarise, #summarize cell counts
                 N    = length(relabund[!is.na(relabund)]), #calculate the length of the data frame, excluding NA’s
                 mean = mean(relabund, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(relabund, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
#mcap no clade
mcap.16s.fam.T01F.top10.m_mean.noC <- ddply(mcap.16s.fam.T01f.top10.m, c("Treatment", "Time.Point","family"), summarise, #summarize cell counts
                 N    = length(relabund[!is.na(relabund)]), #calculate the length of the data frame, excluding NA’s
                 mean = mean(relabund, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(relabund, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
#Means no phylotype
pcomp.16s.fam.top10.m_mean.NoC <- ddply(pcomp.16s.fam.top10.m, c("Treatment", "Time.Point","family"), summarise, #summarize cell counts
                 N    = length(relabund[!is.na(relabund)]), #calculate the length of the data frame, excluding NA’s
                 mean = mean(relabund, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(relabund, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
 ...
 #try plotting only the top 10 FAMs  ####   
    ...
    top10P.names.Pvar = sort(tapply(taxa_sums(pvar.f.f.bac.T01F), tax_table(pvar.f.f.bac.T01F)[, "Family"], sum), TRUE)[1:10]
      ...
      #rename Fam with taxa, all same as T1F
            ...
            names(Pvar.u.f.bac.T01F.top10P.OTU.df)[names(Pvar.u.f.bac.T01F.top10P.OTU.df) == "Otu00270"] <- "uncultured_Otu00270"
            names(Pvar.u.f.bac.T01F.top10P.OTU.df)[names(Pvar.u.f.bac.T01F.top10P.OTU.df) == "Otu00533"] <- "uncultured_Otu00533"
            names(Pvar.u.f.bac.T01F.top10P.OTU.df)[names(Pvar.u.f.bac.T01F.top10P.OTU.df) == "Otu00563"] <- "uncultured_Otu00563"
            names(Pvar.u.f.bac.T01F.top10P.OTU.df)[names(Pvar.u.f.bac.T01F.top10P.OTU.df) == "Otu01647"] <- "uncultured_Otu01647"
            names(Pvar.u.f.bac.T01F.top10P.OTU.df)[names(Pvar.u.f.bac.T01F.top10P.OTU.df) == "Otu00872"] <- "uncultured_Otu00872"
...
#Means no phylotype
pvar.16s.fam.top10.m_mean.NoC <- ddply(Pvar.16s.fam.top10.m, c("Treatment", "Time.Point","family"), summarise, #summarize cell counts
                 N    = length(relabund[!is.na(relabund)]), #calculate the length of the data frame, excluding NA’s
                 mean = mean(relabund, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(relabund, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
pvar.16s.fam.top10.m_mean.NoC$Code<-paste(pvar.16s.fam.top10.m_mean.NoC$Treatment, pvar.16s.fam.top10.m_mean.NoC$Time.Point,pvar.16s.fam.top10.m_mean.NoC$Clade)
...
#Means no phylotype
pacu.16s.fam.top10.m_mean.NoC <- ddply(Pacu.16s.fam.top10.m, c("Treatment", "Time.Point","family"), summarise, #summarize cell counts
                 N    = length(relabund[!is.na(relabund)]), #calculate the length of the data frame, excluding NA’s
                 mean = mean(relabund, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(relabund, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
Mcap_Bac.s.melt <- psmelt(Mcap_phyFam)
...
mcap.p <- ggplot(Mcap_Bac.s.melt, aes(x = Parent.ID, y = Abundance, fill = Family)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values=mycolors) +
  ylab("Relative Abundance") +
  xlab("Sample Label") +
...
Pcomp_Bac.s.melt <- psmelt(Pcomp_phyFam)
...
Pcomp.p <- ggplot(Pcomp_Bac.s.melt, aes(x = Parent.ID, y = Abundance, fill = Family)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values=mycolors) +
  ylab("Relative Abundance") +
  xlab("Sample Label") +
...
Pvar_Bac.s.melt <- psmelt(Pvar_phyFam)
...
Pvar.p <- ggplot(Pvar_Bac.s.melt, aes(x = Parent.ID, y = Abundance, fill = Family)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values=mycolors) +
  ylab("Relative Abundance") +
  xlab("Sample Label") +
...
Pacu_Bac.s.melt <- psmelt(Pacu_phyFam)

#set 
Pacu_Bac.s.melt$Code<-paste(Pacu_Bac.s.melt$Treatment, Pacu_Bac.s.melt$Time.Point)
PPacu_Bac.s.melt$Parent.ID <- factor(Pacu_Bac.s.melt$Parent.ID, levels = c("304",  "308","312","303", "306","302",  "310", "301","311", "307", "309","305"))
Pacu_Bac.s.melt$Code <- factor(Pacu_Bac.s.melt$Code, levels = c("Ambient T0",  "Ambient T1",
                                                                    "Ambient TF","High T0",  "High T1","High TF"))
#FAMILY COLORS
nb.cols <- 252
...
Pacu.p <- ggplot(Pacu_Bac.s.melt, aes(x = Parent.ID, y = Abundance, fill = Family)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values=mycolors) +
  ylab("Relative Abundance") +
  xlab("Sample Label") +
