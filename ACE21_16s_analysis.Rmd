...
Mcap_Bac.s2.Cla <- Mcap_Bac.s2 %>% 
  tax_glom(taxrank = "Class", NArm = FALSE) %>% 
  transform_sample_counts(function(x) {x/sum(x)} ) 
Mcap_Bac.s2.melt <- psmelt(Mcap_Bac.s2.Cla)
...
mcap.p <- ggplot(Mcap_Bac.s2.melt, aes(x = Parent.ID, y = Abundance, fill = Class)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values=mycolors) +
  ylab("Relative Abundance") +
  xlab("Sample Label") +
...
mcap.p <- ggplot(Mcap_Bac.s2.melt.C, aes(x = Parent.ID, y = Abundance, fill = Class)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values=mycolors) +
  ylab("Relative Abundance") +
  xlab("Sample Label") +
...
mcap.p <- ggplot(Mcap_Bac.s2.melt.CD, aes(x = Parent.ID, y = Abundance, fill = Class)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values=mycolors) +
  ylab("Relative Abundance") +
  xlab("Sample Label") +
...
#try glom use raw counts Bac.s2
Mcap_Bac.s2.Fam <- Mcap_Bac.s2 %>% 
  tax_glom(taxrank = "Family", NArm = FALSE) %>% 
  transform_sample_counts(function(x) {x/sum(x)} ) 
Mcap_Bac.s2.melt <- psmelt(Mcap_Bac.s2.Fam)


#before I melt it, do phyloseq to DF command

testFAM<-phyloseq_to_df(Mcap_Bac.s2.Fam, addtax = T, addtot = F, addmaxrank = F,
  sorting = "abundance")


#### try doing tax_glom in one step, then the second one. 
# Mcap_Bac.s2.FamT <- Mcap_Bac.s2 %>% 
#   tax_glom(taxrank = "Family", NArm = FALSE)



Mcap_Bac.s2.FamT<-tax_glom(Mcap_Bac.s2, "Family")



testFAM2<-phyloseq_to_df(Mcap_Bac.s2.FamT, addtax = T, addtot = F, addmaxrank = F,
  sorting = "abundance")


#Get the dataframe in to a usable format. DF will be converted to long format
df_Long <- testFAM2 %>%
  gather(Sample.ID, value, 9:ncol(testFAM2))%>%
  arrange(Family) %>%
  transmute(Sample.ID = Sample.ID, Family = str_c(Domain, Phylum, Class, Order, Family, sep = "_"), value = value) %>%
  #spread(Family, value)
#pivot_wider(names_from = "Family", values_from = "value")

df_Long3<-aggregate(df_Long$value, by=list(Category=df_Long$Family), FUN=sum)
...
df_LongPhy <- testFAM2 %>%
  gather(Sample.ID, value, 9:ncol(testFAM2))%>%
  arrange(Phylum) %>%
  transmute(Sample.ID = Sample.ID, Phylum = str_c(Domain, Phylum, sep = "_"), value = value) %>%
  #spread(Phylum, value)
pivot_wider(names_from = "Phylum", values_from = "value")


#make the dfcreate the code, agg, then convert to long Mcap_Bac.s2

Family.test<-phyloseq_to_df(Mcap_Bac.s2, addtax = T, addtot = F, addmaxrank = F,
  sorting = "abundance")

Family.test2<-Family.test[-c("Phyulum","Class","Order","Genus","Size")] # keep only columns you need


library(dplyr)
Family.test3 = select(Family.test, -Size) #remove all but family
Family.test3 = select(Family.test3, -Domain)
Family.test3 = select(Family.test3, -Phylum)
Family.test3 = select(Family.test3, -Order)
Family.test3 = select(Family.test3, -Class)
Family.test3 = select(Family.test3, -Genus)
Family.test3 = select(Family.test3, -OTU)

Family.test4<-aggregate(Family.test3, by=Family, FUN=sum)

#fuck this so hard.None of this is working
...
Pcomp_Bac.s2.Cla <- Pcomp_Bac.s2 %>% 
  tax_glom(taxrank = "Class", NArm = FALSE) %>% 
  transform_sample_counts(function(x) {x/sum(x)} ) 
Pcomp_Bac.s2.melt <- psmelt(Pcomp_Bac.s2.Cla)

Pcomp_Bac.s2.melt$Code<-paste(Pcomp_Bac.s2.melt$Treatment, Pcomp_Bac.s2.melt$Time.Point)
Pcomp_Bac.s2.melt$Parent.ID <- factor(Pcomp_Bac.s2.melt$Parent.ID, levels = c("363",  "372","373","371", "367","370","364",  "365",
                                                                    "366","368",  "369","374"))
Pcomp_Bac.s2.melt$Code <- factor(Pcomp_Bac.s2.melt$Code, levels = c("Ambient T0",  "Ambient T1",
                                                                    "Ambient TF","High T0",  "High T1","High TF"))
nb.cols <- 123
...
Pcomp.p <- ggplot(Pcomp_Bac.s2.melt, aes(x = Parent.ID, y = Abundance, fill = Class)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values=mycolors) +
  ylab("Relative Abundance") +
  xlab("Sample Label") +
...
Pcomp.p <- ggplot(Pcomp_Bac.s2.melt.C, aes(x = Parent.ID, y = Abundance, fill = Class)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values=mycolors) +
  ylab("Relative Abundance") +
  xlab("Sample Label") +
...
Pcomp.p <- ggplot(Pcomp_Bac.s2.melt.CD, aes(x = Parent.ID, y = Abundance, fill = Class)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values=mycolors) +
  ylab("Relative Abundance") +
  xlab("Sample Label") +
...
```{r}
#try glom use raw counts Bac.s2
Pcomp_Bac.s2.Fam <- Pcomp_Bac.s2 %>% 
  tax_glom(taxrank = "Family", NArm = FALSE) %>% 
  transform_sample_counts(function(x) {x/sum(x)} ) 
Pcomp_Bac.s2.melt <- psmelt(Pcomp_Bac.s2.Fam)
...
All Species Bac.r.rel .  relative abundances 
```{r}        
#Bac.r.rel 
