...
setdiff(ITS.T0.sd.samps,bac.T0.sd.samps) #samples in ITS but not 16s: S208, Sn60, SN70, Sn11, SN40, Sn111, SN56, Sn109
# in ITS change sample S208b name to S208

  # 16s changes: rename sample S208b = S208
    as(sample_data(Bac.s.rel.u.T0), "BacT0.samps.edit")
    
  OTU.16sT0 = as(otu_table(Bac.s.rel.u.T0), "matrix")
  Tax.16sT0 = as(tax_table(Bac.s.rel.u.T0), "matrix")
  aSD.16sT0 = as(sample_data(Bac.s.rel.u.T0), "matrix")
    aSD.16sT0<-as.data.frame(aSD.16sT0) #change to df
    aSD.16sT0$Frag.ID[aSD.16sT0$Frag.ID == "S208b"] <- "S208"  #replace name S208b
        aSD.16sT0$sample_name.1[aSD.16sT0$sample_name.1 == "S208b"] <- "S208"  #replace name S208b
        
  #remake the phyloseq obj
        OTU.16sT0 <- phyloseq(otu, tax, sam)

 
   #ITS2 remove samps not in the 16s df
        Bac.seq <- subset_samples(Bac.seq, sample_name.1 != "S1753b" )
