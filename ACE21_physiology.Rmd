...
Mcap_PAM_plot<-ggplot(data=Mcap_PAM_mean.clade, aes(x=Days, y=mean, group = Code, color=Code)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Code, shape=Clade), size=6, show.legend = T)+  
  geom_line(aes(color=Code),linetype="dotted")+
  scale_color_manual(values=c("dodgerblue3", "darkred","blue","red"))+
...
apDataT0<-subset(apDataT0, select=-c(Calcification))
apDataT0<-na.omit(apDataT0)
apDataT02 <- prcomp(apDataT0[c(14:21)], center = TRUE, scale = TRUE)
summary(apDataT02)
...
ggbiplot(pcobj = apDataT02,
         choices=c(1,2),
         ellipse=TRUE, groups=Miss_scores_T0$species, geom="polygon", ellipse_fill=T, alpha=0.2)+
  theme_classic()
...
AA_PhysioT0<-AA_PhysioT0[,c("Parent.ID","Treatment","Species", "Time.Point","AFDWmg.cm2","zoox.cm2_log","Chl a","Protein", "Fv/Fm","Photosynthesis", "Respiration","PR")] # keep only columns you need
