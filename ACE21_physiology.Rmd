...
Bac.s2RelA  = transform_sample_counts(Bac.s2, function(x) x / sum(x) )  #save as RELA DIVs
data.bac <- as(sample_data(Bac.s2RelA), "data.frame") #look at samp data
...
16s - bar plots df Bac.s2RelA 
```{r} 
#ordination time - CUT WHEN YOU HAVE OTHER 4 SPECIES ORDINATION SORTED 
Bac.s2RelA.ord <- ordinate(Bac.s2RelA, "NMDS", "bray", set.seed(30))
Bac.RelA.ord_plot = plot_ordination(Bac.s2RelA, Bac.s2RelA.ord, type="samples", shape="Treatment",color="Species", title="species")
print(Bac.RelA.ord_plot)
Bac.RelA.ord_plot + facet_wrap(~Species, 3)

### let's do this again and run the NMDS by species now that we know they are different. 
#subset by species
Mcap_16s_RA <- subset_samples(Bac.s2RelA, Species=="Montipora_capitata")
Pcomp_16s_RA <- subset_samples(Bac.s2RelA, Species=="Porites_compressa")
Pvar_16s_RA <- subset_samples(Bac.s2RelA, Species=="Pavona_varians")
Pacu_16s_RA <- subset_samples(Bac.s2RelA, Species=="Pocillopora_acuta")
...
McapBarPlot<-plot_bar(Mcap_16s_RA, x="Parent.ID", fill="Class", facet_grid = "Time.Point~Treatment");McapBarPlot
#ggsave("mMcapBarPlot_16srelARare.pdf", McapBarPlot, dpi=300, width=8)

#Mcap C and CD
#C
Mcap_16s_RA.C<-subset_samples(Mcap_16s_RA, Clade=="C")
TopNOTUsMcap.C = names(sort(taxa_sums(Mcap_16s_RA.C), TRUE)[1:50])
Mcap.50.C = prune_taxa(TopNOTUsMcap.C, Mcap_16s_RA.C)
McapBarPlot.C<-plot_bar(Mcap_16s_RA.C, x="Parent.ID", fill="Phyla", facet_grid = "Time.Point~Treatment");McapBarPlot.C
#ggsave("McapBarPlot_16s_C.pdf", McapBarPlot.C, dpi=300, width=8)

#CD
Mcap_16s_RA.CD<-subset_samples(Mcap_16s_RA, Clade=="CD")
TopNOTUsMcap.CD = names(sort(taxa_sums(Mcap_16s_RA.CD), TRUE)[1:50])
 Mcap.50.CD = prune_taxa(TopNOTUsMcap.CD, Mcap_16s_RA.CD)
McapBarPlot.CD<-plot_bar(Mcap_16s_RA.CD, x="Parent.ID", fill="Phyla", facet_grid = "Time.Point~Treatment");McapBarPlot.CD
#ggsave("McapBarPlot_16s_CD.pdf", McapBarPlot.CD, dpi=300, width=8)


#Pcomp
TopNOTUsPcomp = names(sort(taxa_sums(Pcomp_16s_RA), TRUE)[1:50])
# Pcompent50 = prune_taxa(TopNOTUsPcomp,Pcomp_16s_RA )
PcompBarPlot<-plot_bar(Pcomp_16s_RA, x="Parent.ID", fill="Phyla", facet_grid = "Time.Point~Treatment");PcompBarPlot
#ggsave("PcompBarPlot_16srelARare.pdf", PcompBarPlot, dpi=300, width=8)

#Pvar
TopNOTUsVar = names(sort(taxa_sums(Pvar_16s_RA), TRUE)[1:50])
Pvarent50 = prune_taxa(TopNOTUsVar, Pvar_16s_RA)
PvarBarPlot<-plot_bar(Pvarent50, x="Parent.ID", fill="16s", facet_grid = "Time.Point~Treatment");PvarBarPlot
#ggsave("PvarBarPlot_16srelARare.pdf", PvarBarPlot, dpi=300, width=8)

#Pacu
TopNOTUsPacu = names(sort(taxa_sums(Pacu_16s_RA), TRUE)[1:50])
Pacuent50 = prune_taxa(TopNOTUsPacu, Pacu_16s_RA)
PacuBarPlot<-plot_bar(Pacuent50, x="Parent.ID", fill="16s", facet_grid = "Time.Point~Treatment");PacuBarPlot
#ggsave("PacuBarPlot_16srelARare.pdf", PacuBarPlot, dpi=300, width=8)
```


#Ordination DIV_RelA <- you might not need this section, code below for each species  
try vegan nmds - its the same, delete when CN says cool, Bac.s2RelA
```{r} 
  ##this isn't working, not converging. 
# convert the sample_data() within a phyloseq object to a vegan compatible data object
df16s2veg <- function(Bac.s2RelA) { 
  sd <- sample_data(Bac.s2RelA)
  return(as(sd,"data.frame"))
}

# convert the otu_table() within a phyloseq object to a vegan compatible data object
df16sotu2veg <- function(Bac.s2RelA) {
  OTU <- otu_table(Bac.s2RelA)
  if (taxa_are_rows(OTU)) {
    OTU <- t(OTU)
  }
  return(as(OTU, "matrix"))
...
#heat map
plot_heatmap(Bac.s2RelA, "NMDS", "bray", "Species", "Phyla")
...
Mcapent50.TF<- subset_samples(Mcapent50, Time.Point=="TF")
  ...
  plot_heatmap(Mcapent50.T1, "NMDS", "bray", "Treatment","Phyla", low="#000033", high="#CCFF66") #use to show split
  plot_heatmap(Mcapent50.T1.C, "NMDS", "bray", "Treatment","Phyla", low="#000033", high="#CCFF66") #use to show split
  plot_heatmap(Mcapent50.T1.CD, "NMDS", "bray", "Treatment","Phyla", low="#000033", high="#CCFF66") #use to show split
...
#Beta Diversity: df rarefied and relative abudnaces  
All Species Bac.s2RelA
```{r}      
#Bac.s2RelA 

Bac.s2RelA_stats<- Bac.s2RelA #make a copy
otu_table(Bac.s2RelA_stats)[1:5, 1:5] #check rela worked

#get rid of NA OTUs now!
Bac.s2RelA_stats = prune_taxa(taxa_sums(Bac.s2RelA_stats) > 0, Bac.s2RelA_stats) #this removes any OTU with 0s


#bray curtis presence-absense and abundance
## REMOVE T0 for models
Bac_RelA_stats.T1F<-subset_samples(Bac.s2RelA_stats, Time.Point=="T1"|Time.Point=="TF")
...
scores.ord.Bac_RelA_stats <- as.data.frame(scores(ord.Bac_RelA_stats)) #HEY when this does not work it is bc you have NAs from your subset. go back and figure out how to remove OTUs that are now not a part of the phylo
...
#make bray curtis data matrix -T1 and TF
  ...
  #heatmap
    #plot_heatmap(mcap.bac.T1F, "NMDS", "bray", "Parent.ID", "Phyla")

  
#T1
#make bray curtis data matrix 
  bc.mcapT1 <- phyloseq::distance(mcap.bac.T1, method = "bray")
  mcap.samp.df.T1 <- as(sample_data(mcap.bac.T1), "data.frame") #sample df
  adonis(bc.mcapT1 ~ Clade*Treatment, data = mcap.samp.df.T1)   #adonis test
# Homogeneity of dispersion test  
  beta.mcap.T1 <- betadisper(bc.mcapT1, mcap.samp.df.T1$Clade)
  permutest(beta.mcap.T1, pairwise = T) 
...
#make bray curtis data matrix 
  ...
  mcap.samp.df.TF <- as(sample_data(mcap.bac.TF), "data.frame") #sample df
#adonis test
  adonis(bc.mcap.TF ~ Clade*Treatment, data = mcap.samp.df.TF)
  # Homogeneity of dispersion test  
  beta.mcap.TF <- betadisper(bc.mcap.TF, mcap.samp.df.TF$Clade)
...
# C only, new objs
mcap.bacT1F.C <- subset_samples(mcap.bac.T1F, Clade == "C")
mcap.bac.T1.C <- subset_samples(mcap.bac.T1, Clade == "C")
mcap.bac.TF.C <- subset_samples(mcap.bac.TF, Clade == "C")
  
#T1TF.C
#make bray curtis data matrix -T1 and TF
  bc.mcap.T1F.C <- phyloseq::distance(mcap.bacT1F.C, method = "bray")
  mcap.samp.df.T1F.C <- as(sample_data(mcap.bacT1F.C), "data.frame") #sample df
#adonis test
  adonis(bc.mcap.T1F.C ~ Treatment*Time.Point, data = mcap.samp.df.T1F.C)
...
# CD only, new objs mcap.bac.TF
mcap.bacT1F.CD <- subset_samples(mcap.bac.T1F, Clade == "CD")
mcap.bac.T1.CD <- subset_samples(mcap.bac.T1, Clade == "CD")
mcap.bac.TF.CD <- subset_samples(mcap.bac.TF, Clade == "CD")
  
#T1TF.CD
#make bray curtis data matrix -T1 and TF
  bc.mcap.T1F.CD <- phyloseq::distance(mcap.bacT1F.CD, method = "bray")
  mcap.samp.df.T1F.CD <- as(sample_data(mcap.bacT1F.CD), "data.frame") #sample df
#adonis test
  adonis(bc.mcap.T1F.CD ~ Treatment*Time.Point, data = mcap.samp.df.T1F.CD)
...
#make bray curtis data matrix 
  ...
  mcap.samp.df.T1.CD <- as(sample_data(mcap.bac.T1.CD), "data.frame") #sample df
  adonis(bc.mcapT1.CD ~ Treatment, data = mcap.samp.df.T1.CD)#adonis test
# Homogeneity of dispersion test  
  beta.mcap.T1.CD <- betadisper(bc.mcapT1.CD, mcap.samp.df.T1.CD$Treatment)
  permutest(beta.mcap.T1.CD, pairwise = T) 
...
#make bray curtis data matrix 
  ...
  mcap.samp.df.TF.CD <- as(sample_data(mcap.bac.TF.CD), "data.frame") #sample df
#adonis test
  adonis(bc.mcap.TF.CD ~ Treatment, data = mcap.samp.df.TF.CD)
# Homogeneity of dispersion test  
  beta.mcap.TF.CD <- betadisper(bc.mcap.TF.CD, mcap.samp.df.TF.CD$Treatment)
...
ggplot(scores.mcap.samp.df, aes(x = NMDS1, y = NMDS2)) + 
  ...
  #scale_color_manual(values=Mcap.Parent.colors)+
   # geom_line(aes(group = Code2), color = "black") + 
...
ggplot(scores.mcap_C, aes(x = NMDS1, y = NMDS2)) + 
geom_point(aes(stroke=3, colour = Parent.ID, shape=Time.Point,size = 4) )+
    geom_point(aes(colour = Parent.ID, size = 4, shape=Time.Point)) +
  scale_color_manual(values=Mcap.Parent.colors)+
  stat_ellipse(aes(x = NMDS1, y = NMDS2, colour = Treatment), linetype = 2) +
  ggtitle("M. capitata Cladocopium") +
  theme_classic()
