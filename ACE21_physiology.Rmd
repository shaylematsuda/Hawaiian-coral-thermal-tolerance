...
emm<-emmeans(PAMdata_stats_model,  ~ Days*Treatment|Species, adjust="Tukey")
cld(emm)
pairs(emm)
...
PhysioData_noMissing<-na.omit(PhysioData) #this removes 9 samples (partent/time/species)
# 1 AFDW
# 3 Flow Cy
# 2 prot  N107 pcomp t0 
...
##You can make hulls for your data clouds to group them by a variable
Phy_MY.hull <- Phy_nmds_Mcap.MDS.scores %>% group_by(Treatment) %>%
  do({
    x = .
    data.frame(x[chull(x$NMDS1, x$NMDS2), ])
  })
...
#Make your ellipse:
phy_ell <- data.frame()
for(g in levels(Phy_nmds_Mcap.MDS.scores$Treatment)){
  phy_ell <- rbind(phy_ell, cbind(as.data.frame(with(Phy_nmds_Mcap.MDS.scores[Phy_nmds_Mcap.MDS.scores$Treatment==g,],
                                                   veganCovEllipse(cov.wt(cbind(NMDS1,NMDS2),wt=rep(1/length(NMDS1),length(NMDS1)))$cov,center=c(mean(NMDS1),mean(NMDS2)))))
                                ,group=g))
}
...
##With hulls:
#ggplot(Phy_nmds_Mcap.MDS.scores, aes(x = NMDS1, y= NMDS2)) + geom_point(aes(colour = Species)) +
#  geom_polygon(data = Phy_MY.hull, aes(x = NMDS1, y = NMDS2, colour= Code2, fill = Species), alpha = 0.1) #+
#  theme_classic() 

##With ellipses:

ggplot(Phy_nmds_Mcap.MDS.scores, aes(x = NMDS1, y= NMDS2)) + geom_point(aes(colour = Time.Point, shape =Time.Point)) +
  geom_path(data = phy_ell, aes(x = NMDS1, y = NMDS2, colour= group),size = 0.5, linetype = 1) +
  theme_classic()+
...
phy_ell <- data.frame()
for(g in levels(Phy_nmds_Pcomp.MDS.scores$Treatment)){
  phy_ell <- rbind(phy_ell, cbind(as.data.frame(with(Phy_nmds_Pcomp.MDS.scores[Phy_nmds_Pcomp.MDS.scores$Treatment==g,],
                                                   veganCovEllipse(cov.wt(cbind(NMDS1,NMDS2),wt=rep(1/length(NMDS1),length(NMDS1)))$cov,center=c(mean(NMDS1),mean(NMDS2)))))
                                ,group=g))
}
#Check that the command worked - often I run into issues if the variable is not a factor (important to check as above!)
...
ggplot(Phy_nmds_Pcomp.MDS.scores, aes(x = NMDS1, y= NMDS2)) + geom_point(aes(colour = Treatment, shape =Time.Point)) +
  geom_path(data = phy_ell, aes(x = NMDS1, y = NMDS2, colour= group),size = 0.5, linetype = 1) +
  theme_classic()+
...
ggplot(Phy_nmds_Pcomp.MDS.scores, aes(x = NMDS1, y= NMDS2)) + geom_point(aes(colour = Code2, shape =Code2)) +
  geom_path(data = phy_ell, aes(x = NMDS1, y = NMDS2, colour= group),size = 0.5, linetype = 1) +
  theme_classic()+
  ggtitle("NMDS Pcomp Time+Treatment")
...
##You can make hulls for your data clouds to group them by a variable
#Phy_MY.hull <- Phy_nmds_Pvar.MDS.scores %>% group_by(Treatment) %>%
#  do({
#    x = .
#    data.frame(x[chull(x$NMDS1, x$NMDS2), ])
#  })
...
## make 95% ellipses using the following commands: CODE2
phy_ell <- data.frame()
for(g in levels(Phy_nmds_Pvar.MDS.scores$Treatment)){
  phy_ell <- rbind(phy_ell, cbind(as.data.frame(with(Phy_nmds_Pvar.MDS.scores[Phy_nmds_Pvar.MDS.scores$Treatment==g,],
                                                   veganCovEllipse(cov.wt(cbind(NMDS1,NMDS2),wt=rep(1/length(NMDS1),length(NMDS1)))$cov,center=c(mean(NMDS1),mean(NMDS2)))))
                                ,group=g))
}
#Check that the command worked - often I run into issues if the variable is not a factor (important to check as above!)
...
ggplot(Phy_nmds_Pvar.MDS.scores, aes(x = NMDS1, y= NMDS2)) + geom_point(aes(colour = Treatment, shape =Time.Point)) +
  geom_path(data = phy_ell, aes(x = NMDS1, y = NMDS2, colour= group),size = 0.5, linetype = 1) +
  theme_classic()+
...
## make 95% ellipses using the following commands: CODE2
phy_ell <- data.frame()
for(g in levels(Phy_nmds_Pacu.MDS.scores$Treatment)){
  phy_ell <- rbind(phy_ell, cbind(as.data.frame(with(Phy_nmds_Pacu.MDS.scores[Phy_nmds_Pacu.MDS.scores$Treatment==g,],
                                                   veganCovEllipse(cov.wt(cbind(NMDS1,NMDS2),wt=rep(1/length(NMDS1),length(NMDS1)))$cov,center=c(mean(NMDS1),mean(NMDS2)))))
                                ,group=g))
}
#Check that the command worked - often I run into issues if the variable is not a factor (important to check as above!)
...
ggplot(Phy_nmds_Pacu.MDS.scores, aes(x = NMDS1, y= NMDS2)) + geom_point(aes(colour = Treatment, shape =Time.Point)) +
  geom_path(data = phy_ell, aes(x = NMDS1, y = NMDS2, colour= group),size = 0.5, linetype = 1) +
  theme_classic()+
...
#not sure how this works but the internet suggested it.
adonis2(PhysioData_noMissing [7:12] ~ Species*Treatment*Time.Point, data = PhysioData_noMissing [1:6], permutations = 999, method="bray", by = NULL)

#this isnt' running for a single effect or multiple
...
#krissy code
adonis(PhysioData_noMissing [7:12]~ Species + Treatment+ Time.Point+ Species:Treatment:Time.Point, data=PhysioData_noMissing [1:6], contr.unordered = "contr.treatment", by = "margin", distance = "bray")
#all significant. 
...
#not sure how this works but the internet suggested it.
adonis2(Phy_nmds_Mcap [7:12] ~ Treatment*Time.Point, data = Phy_nmds_Mcap [1:6], permutations = 999, method="bray", by = NULL)
...
#not sure how this works but the internet suggested it.
adonis2(Phy_nmds_Pcomp [7:12] ~ Treatment*Time.Point, data = Phy_nmds_Pcomp [1:6], permutations = 999, method="bray", by = NULL)
...
#not sure how this works but the internet suggested it.
adonis2(Phy_nmds_Pvar [7:12] ~ Treatment*Time.Point, data = Phy_nmds_Pvar [1:6], permutations = 999, method="bray", by = NULL)
...
#not sure how this works but the internet suggested it.
adonis2(Phy_nmds_Pacu [7:12] ~ Treatment*Time.Point, data = Phy_nmds_Pacu [1:6], permutations = 999, method="bray", by = NULL)
...
coralDIV2_rare2
coralDIV2_rare2View <- as(sample_data(coralDIV2_rare2), "data.frame") #create sample data frame to view

  
```

# DIV analysis and plots, use rarefied coralDIV2_rare
```{r} 
#if you want to transform to REL Abund, do after filtering?
...
#top 20 DIVs across by species 

#Mcap
TopNOTUsMcap = names(sort(taxa_sums(Mcap_DIV_RA), TRUE)[1:50])
Mcapent50 = prune_taxa(TopNOTUsMcap, Mcap_DIV_RA)
TopNOUTsMcap_plot<-plot_bar(Mcapent50, "sample_name",fill = "DIV", facet_grid = ~DIV)
McapBarPlot<-plot_bar(Mcapent50, x="Parent.ID", fill="DIV", facet_grid = "Time.Point~Treatment")
...
TopNOUTsPcomp_plot<-plot_bar(Pcompent50, "sample_name",fill = "DIV" )
PcompBarPlot<-plot_bar(Pcompent50, x="Parent.ID", fill="DIV", facet_grid = "Time.Point~Treatment")
...
TopNOUTsPvar_plot<-plot_bar(Pvarent50, "Parent.ID",fill = "DIV",   facet_grid = ~DIV)
PvarBarPlot<-plot_bar(Pvarent50, x="Parent.ID", fill="DIV", facet_grid = "Time.Point~Treatment")
...
TopNOUTsPacu_plot<-plot_bar(Pacuent50, "Parent.ID",fill = "DIV",  ffacet_grid = ~DIV)
PacuBarPlot<-plot_bar(Pacuent50, x="Parent.ID", fill="DIV", facet_grid = "Time.Point~Treatment")

ggsave("PacuBarPlot_DIVrelARare.pdf", PacuBarPlot, dpi=300, width=8)

#everything and not useful
#plot_bar(DIV_RelA, "Species", fill="DIV", facet_grid=Treatment~Time.Point)
...
```{r}  
OrdiSpecies<-plot_ordination(DIV_RelA, ordinate(DIV_RelA, "NMDS"), color = "Species", label="ID") + geom_point(size = 5) 
...
#Add an ellipse for T0
ordiellipse(DIVrareRel1_Pcomp, groups=meta$Time.Point, display="species", kind="se", conf=0.99, label=FALSE, col="green", draw="polygon", alpha=200, show.groups = c("Treatment"), border=FALSE)




########ok you are here ^ figure out how to do this ordination stuff. 

```
#Beta Diversity: df rarefied and relative abudnaces
```{r} 
DIV_RelA_stats  = transform_sample_counts(coralDIV2_rare, function(x) x / sum(x) )  #save as RELA DIVs
...
#Species                        3    62.187 20.7289 161.051 0.70057  0.001 ***
#Treatment                      1     0.056  0.0559   0.435 0.00063  0.837    
#Time.Point                     2     0.034  0.0170   0.132 0.00038  1.000    
#Species:Treatment              3     0.147  0.0489   0.380 0.00165  0.990    
#Species:Time.Point             6     0.102  0.0169   0.132 0.00114  1.000    
#Treatment:Time.Point           2     0.086  0.0432   0.336 0.00097  0.977    
#Species:Treatment:Time.Point   6     0.284  0.0474   0.368 0.00320  1.000    
#Residuals                    201    25.871  0.1287         0.29145           
#Total                        224    88.766                 1.00000           

# Homogeneity of dispersion test
betaAll <- betadisper(bc.all, samp.df$Species)
permutest(betaAll)  #p = 0.001. This means the differences could be due to group dispersions NOT our main effects
...
scores.ord.DIV_RelA_stats <- as.data.frame(scores(ord.DIV_RelA_stats))
...
```{r}
#mcap
mcap.RelRare <- subset_samples(DIV_RelA_stats, host_genus == "montipora")

#all speices
set.seed(30)

#make bray curtis data matrix
bc.mcap <- phyloseq::distance(mcap.RelRare, method = "bray")
  mcap.samp.df <- as(sample_data(mcap.RelRare), "data.frame") #sample df
#adonis test
adonis(bc.mcap ~ Treatment*Time.Point, data = mcap.samp.df)

#                     Df SumsOfSqs  MeanSqs F.Model      R2 Pr(>F)
#Treatment             1    0.0518 0.051762 0.48017 0.00942  0.640
#Time.Point            2    0.0408 0.020420 0.18942 0.00743  0.983
#Treatment:Time.Point  2    0.1184 0.059186 0.54903 0.02155  0.725
#Residuals            49    5.2822 0.107800         0.96159       
#Total                54    5.4932                  1.00000   

# Homogeneity of dispersion test  <- do I need to do this if ^^ not significant?
beta.mcap <- betadisper(bc.mcap, mcap.samp.df$Treatment)
permutest(beta.mcap)  #p = 0.001. This means the differences could be due to group dispersions NOT our main effects
permutest(beta.mcap, pairwise = T) 

(bc.mcap.HSD <- TukeyHSD(beta.mcap)) #do you need?
plot(bc.mcap.HSD)




# Mcap by timepoint: T1
mcap.RelRare.T1 <- subset_samples(mcap.RelRare, Time.Point == "T1")
...
#adonis test
adonis(bc.mcap.T1 ~ Treatment, data = mcap.samp.df.T1) # p=.976


# Mcap by timepoint: TF
mcap.RelRare.TF <- subset_samples(mcap.RelRare, Time.Point == "TF")
...
#adonis test
adonis(bc.mcap.TF ~ Treatment, data = mcap.samp.df.TF) # p=.777

# High treatment over time
mcap.RelRare.High <- subset_samples(mcap.RelRare, Treatment == "High")

set.seed(30)

#make bray curtis data matrix
bc.mcap.High <- phyloseq::distance(mcap.RelRare.High, method = "bray")
  mcap.samp.df.High <- as(sample_data(mcap.RelRare.High), "data.frame") #sample df
#adonis test
adonis(bc.mcap.High ~ Time.Point, data = mcap.samp.df.High) # p=.714
...
Mcap.Parent.colors <- c("363" = "green4",  "364" = "firebrick3", "365" ="deeppink4", "366" = "orange", "367" = "green1","368" = "gold",  "369"= "hotpink", "370" ="darkslateblue", "371" = "honeydew4", "372" = "khaki4","373" ="ivory4", "374" = "lightblue", Ambient = "Blue", High = "Red") 


#all
set.seed(30)
ord.mcap <- ordinate(mcap.RelRare, "NMDS", "bray", trymax = 100) 
stressplot(ord.mcap)
scores.mcap.samp.df <- as.data.frame(scores(ord.mcap))
scores.mcap.samp.df$Time.Point <- mcap.samp.df$Time.Point
scores.mcap.samp.df$Parent.ID <- mcap.samp.df$Parent.ID
scores.mcap.samp.df$Treatment <- mcap.samp.df$Treatment
...
# #T1 subset full ord, compare = exactly the same
# scores.mcapT1b<-subset(scores.mcap.samp.df,Time.Point=="T1")
# 
# ggplot(scores.mcapT1b, aes(x = NMDS1, y = NMDS2)) + 
#   geom_point(aes(colour = Parent.ID, shape = Treatment), size = 4) +
#   scale_color_manual(values=Mcap.Parent.colors)+
#   #geom_text(label = rownames(scores), nudge_x = 0.05, nudge_y = 0.05,check_overlap = T) +
#   stat_ellipse(aes(x = NMDS1, y = NMDS2, colour = Treatment), linetype = 2) +
...
#                     Df SumsOfSqs   MeanSqs F.Model      R2 Pr(>F)
#Treatment             1   0.00845 0.0084548 0.74955 0.01271  0.572
#Time.Point            2   0.03284 0.0164176 1.45547 0.04938  0.169
#Treatment:Time.Point  2   0.01454 0.0072714 0.64463 0.02187  0.770
#Residuals            54   0.60911 0.0112799         0.91603       
#Total                59   0.66495                   1.00000       

# Homogeneity of dispersion test  <- do I need to do this if ^^ not significant?
beta.pcomp <- betadisper(bc.pcomp, pcomp.samp.df$Treatment)
...
# High treatment over time
pcomp.RelRare.High <- subset_samples(pcomp.RelRare, Treatment == "High")
...
coralDIV2_rare df phyloseq
coralDIV2.rare df sample data
```{r} 
coralDIV2_rare.alpha<-coralDIV2_rare #make a copy of df


coralDIV2_rare.alpha2 <- prune_taxa(taxa_sums(coralDIV2_rare.alpha) > 0, coralDIV2_rare.alpha) #prune taxa 0s

erich <- estimate_richness(coralDIV2_rare, measures = c("Observed", "Shannon")) #select which estimates and estimate
  erich$Treatment <- coralDIV2.rare$Treatment       #add back in meta
  erich$Time.Point <- coralDIV2.rare$Time.Point
  erich$Parent.ID <- coralDIV2.rare$Parent.ID
  erich$ID <- coralDIV2.rare$ID
  erich$Species <- coralDIV2.rare$Species
...
#stats ALL THE DATA
 library(ggpubr)
  #look at data
#observed
hist(erich$Observed) #not normal  <- bimodal,due to species? 
ggqqplot(erich$Observed)
hist(log(erich$Observed)) #not normal
ggqqplot(log(erich$Observed))
shapiro.test(log(erich$Observed))

#shannon
hist(erich$Shannon) #not normal  <- bimodal,due to species? 
ggqqplot(erich$Shannon)
hist(log(erich$Shannon)) #not normal
ggqqplot(log(erich$Shannon))
hist(1/(erich$Shannon)) #not normal

shapiro.test(log(erich$Shannon))
...
erich.mcap<-subset(erich, Species=="Montipora_capitata")
  ...
  ggqqplot(erich.mcap$Observed) #looks good
  hist(log10(erich.mcap$Observed)) #worse
  ggqqplot(log(erich.mcap$Observed))
  hist((erich.mcap$Observed)^1/3) #worse
  ggqqplot(log(erich.mcap$Observed))
...
erich.pcomp<-subset(erich, Species=="Porites_compressa")
  ...
  hist(log(erich.pcomp$Observed)) #not normal
  ggqqplot(log(erich.pcomp$Observed))


erich.pvar<-subset(erich, Species=="Pavona_varians")
 hist(erich.pvar$Observed) #not normal 
  ggqqplot(erich.pvar$Observed) #looks bad
  hist(log(erich.pvar$Observed)) #better, still a little right skew
  ggqqplot(log(erich.pvar$Observed))
  hist(sqrt(erich.pvar$Observed))#b
  ggqqplot(log(erich.pvar$Observed))
...
erich.pacu<-subset(erich, Species=="Pocillopora_acuta")
 hist(erich.pacu$Observed) #not awfuk 
  ...
  hist(log10(erich.pacu$Observed)) #worse
  ggqqplot(log(erich.pacu$Observed))
  hist((erich.pacu$Observed)^1/3)#bad
  ggqqplot(log(erich.pacu$Observed))
  
## subset for Shannon
  
erich.mcap<-subset(erich, Species=="Montipora_capitata")
  hist(erich.mcap$Shannon) #not normal  <- bimodal 
  ggqqplot(erich.mcap$Shannon) #looks good
  hist(log10(erich.mcap$Shannon)) #worse
  ggqqplot(log(erich.mcap$Shannon))
  hist((erich.mcap$Shannon)^1/3) #worse
  ggqqplot(log(erich.mcap$Shannon))
...
erich.pcomp<-subset(erich, Species=="Porites_compressa")
  ...
  hist(log(erich.pcomp$Shannon)) # normal better
  ggqqplot(log(erich.pcomp$Shannon))


erich.pvar<-subset(erich, Species=="Pavona_varians")
 hist(erich.pvar$Shannon) #not normal 
  ggqqplot(erich.pvar$Shannon) #
  hist(log(erich.pvar$Shannon)) #better, still a little right skew
  ggqqplot(log(erich.pvar$Shannon))
...
erich.pacu<-subset(erich, Species=="Pocillopora_acuta")
 hist(erich.pacu$Shannon) #not awfuk 
  ...
  hist(log10(erich.pacu$Shannon)) #worse
  ggqqplot(log(erich.pacu$Shannon))
  hist(asin(sqrt(erich.pacu$Shannon)))#bad
  ggqqplot(log(erich.pacu$Shannon))
...
#Observed
#Run the ANOVA and save it as an object
  aov.observed.species = aov(log(Observed) ~ Species*Treatment*Time.Point, data=erich)
...
#Plot
  boxplot(Observed ~ Species*Treatment*Time.Point, data=erich, ylab="observed's diversity") 
```
...
#Run the ANOVA and save it as an object
  ...
  aov.shannon.speciesMcap = aov(log(Shannon) ~ Treatment*Time.Point, data=erich.mcap)
  summary(aov.shannon.speciesMcap) # sig diff between species
  TukeyHSD(aov.shannon.speciesMcap)
  
    aov.shannon.speciesMcap = aov(Shannon^(1/3) ~ Treatment*Time.Point, data=erich.mcap)
  summary(aov.shannon.speciesMcap) # sig diff between species
  TukeyHSD(aov.shannon.speciesMcap)
...
#Run the ANOVA and save it as an object
  ...
  aov.observed.speciesMcap = aov(log(Observed) ~ Treatment*Time.Point, data=erich.mcap)
  summary(aov.observed.speciesMcap) # sig diff between species
  TukeyHSD(aov.observed.speciesMcap)
  
  aov.observed.speciesMcap = aov(Observed^(1/3) ~ Treatment*Time.Point, data=erich.mcap)
  summary(aov.observed.speciesMcap) # sig diff between species
  TukeyHSD(aov.observed.speciesMcap)
...
#Run the ANOVA and save it as an object
  ...
  aov.shannon.speciesPcomp = aov(log(Shannon) ~ Treatment*Time.Point, data=erich.pcomp)
  summary(aov.shannon.speciesPcomp) # sig diff between species
  TukeyHSD(aov.shannon.speciesPcomp)
...
#Run the ANOVA and save it as an object
  ...
  aov.observed.speciesPcomp = aov(log(Observed) ~ Treatment*Time.Point, data=erich.pcomp)
  summary(aov.observed.speciesPcomp) # sig diff between species
  TukeyHSD(aov.observed.speciesPcomp)
...
#Run the ANOVA and save it as an object
  ...
  aov.shannon.speciesPvar = aov(log(Shannon) ~ Treatment*Time.Point, data=erich.pvar)
  summary(aov.shannon.speciesPvar) # sig diff between species
  TukeyHSD(aov.shannon.speciesPvar)
...
#Run the ANOVA and save it as an object
  ...
  aov.observed.speciesPvar = aov(log(Observed) ~ Treatment*Time.Point, data=erich.pvar)
  summary(aov.observed.speciesPvar) # sig diff between species
  TukeyHSD(aov.observed.speciesPvar)
...
# Just T1
  erich.pvarT1<-subset(erich.pvar, Time.Point=="T1")
  hist(log10(erich.pvarT1$Observed))
  aov.observed.speciesPvarT1 = aov(log10(Observed) ~ Treatment, data=erich.pvarT1)
  summary(aov.observed.speciesPvarT1) # sig diff between species
  TukeyHSD(aov.observed.speciesPvarT1)
  
  hist((erich.pvarT1$Shannon))
  aov.shannon.speciesPvarT1 = aov(log10(Shannon) ~ Treatment, data=erich.pvarT1)
  summary(aov.shannon.speciesPvarT1) # sig diff between species
  TukeyHSD(aov.shannon.speciesPvarT1)
...
#Run the ANOVA and save it as an object
  ...
  aov.shannon.speciesPacu = aov(log(Shannon) ~ Treatment*Time.Point, data=erich.pacu)
  summary(aov.shannon.speciesPacu) # sig diff between species
  TukeyHSD(aov.shannon.speciesPacu)
...
#Run the ANOVA and save it as an object
  ...
  aov.observed.speciesPacu = aov(log(Observed) ~ Treatment*Time.Point, data=erich.pacu)
  summary(aov.observed.speciesPacu) # sig diff between species
  TukeyHSD(aov.observed.speciesPacu)
...
### I dont think we can do this for profiles
