...
pData<-na.omit(pData) #this removes 9 samples (partent/time/species)
...
Phy_nmds.MDS<-metaMDS(Phy_nmds[,11:20], distance = "bray", k = 2, trymax = 200, engine = c("monoMDS", "isoMDS"))
...
vec.sp<-envfit(Phy_nmds.MDS.T1, Phy_nmds.T1[,8:16], perm=1000)
...
ggplot(Phy_nmds.MDS.scores, aes(x = NMDS1, y = NMDS2)) + 
  ...
  stat_ellipse(aes(x = NMDS1, y = NMDS2, colour = Species), linetype = 2) +
    ...
    geom_text(data=vec.sp.df,aes(x=NMDS1,y=NMDS2,label=Code),size=5)+
...
Phy_nmds.MDS.T0<-metaMDS(pData.T0[,(9:16)], distance = "bray", k = 2, trymax = 200, engine = c("monoMDS"))
