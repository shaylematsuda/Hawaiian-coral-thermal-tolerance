...
#for t0 only
pData<-PhysioData
pData <- pData %>% dplyr::select(-Calcification)
pData<-na.omit(pData) #remove anything without a value
...
Phy_nmds.MDS<-metaMDS(Phy_nmds[,8:20], distance = "bray", k = 2, trymax = 200, engine = c("monoMDS", "isoMDS"))
...
apData<-apData[,c("Parent.ID","Treatment","Species", "Time.Point", "Calcification","AFDWmg.cm2","zoox.cm2_log","Chl a","Protein", "Fv/Fm","Photosynthesis", "Respiration","PR")] #put cols in correct order
...
Miss_scores_T0_plot<-ggplot(Miss_scores_T0, aes(x = PC1, y= PC2)) + geom_point(aes(colour = species, shape = species), size = 4) +
  stat_ellipse(aes(x = PC1, y = PC2, colour = species), linetype = 2, level=0.25) +
  theme_classic();Miss_scores_T0_plot
