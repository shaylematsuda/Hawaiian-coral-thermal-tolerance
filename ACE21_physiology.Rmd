...
#T0 only - by algal profile
  ggplot(biomass_T0, aes(x=Species, y=AFDWmg.cm2, fill=Clade, group=interaction(Species, Clade))) + 
      scale_fill_manual(values=c("darkorange2", "pink1","powderblue","#0072B2" ,"firebrick","springgreen4", "palegreen2","goldenrod1"))+
      xlab("") + #Label the X Axis
      ylab(bquote('Ash-free Dry Weight '(mg~cm^2)))+
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
      panel.background = element_blank(), axis.line = element_line(colour = "black")) + 
...
#T1
ggplot(biomass_T1, aes(x=Species, y=AFDWmg.cm2, fill=Treatment)) + 
...
Biomass_means <- summarySE(biomass2, measurevar="AFDWmg.cm2", groupvars=c("Species","Treatment","Time.Point"))

#Subset
biomass_Mcap<-subset(biomass2, Species=="Montipora capitata") 
biomass_Pcomp<-subset(biomass2, Species=="Porites compressa") 
biomass_Pvar<-subset(biomass2, Species=="Pavona varians") 
biomass_Pacu<-subset(biomass2, Species=="Pocillopora acuta") 
...
#write.csv(data_McapBW,"data_McapBW.csv")
