...
PAM_psy$Days[PAM_psy$Days == "17"] <- "T1" #change 
PAM_psy$Days[PAM_psy$Days == "52"] <- "TF" #change 
...
PAM_psy<-PAM_psy[!(PAM_psy$Days=="24"),] #remove other days by name
...
PhysioData_noMissing<-na.omit(PhysioData)
#write.csv(PhysioData,"PhysioData_20210310.csv")
```

nMDS of physio, PhysioData df, by Parent.ID
```{r}
PhysioData_nmds<-PhysioData_noMissing[, c(7:12)]
set.seed(2)

example_NMDS=metaMDS(PhysioData_nmds, # Our community-by-species matrix
                     k=2, na.rm=T) # The number of reduced dimensions
