...
#means
biomass_means.C <- ddply(biomass2, c( "Species","Treatment","Time.Point", "Clade"), summarise, #summarize cell counts
                 N    = length(AFDWmg.cm2[!is.na(AFDWmg.cm2)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                 mean = mean(AFDWmg.cm2, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(AFDWmg.cm2, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
all plots by clade
```{r}
FC_mean.C <- ddply(FC_Data, c( "Species","Treatment","Time.Point","Clade"), summarise, #summarize cell counts
                 N    = length(zoox.cm2[!is.na(zoox.cm2)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                 mean = mean(zoox.cm2, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(zoox.cm2, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
FC_meanlog <- ddply(FC_Data, c( "Species","Treatment","Time.Point"), summarise, #summarize cell counts
                 N    = length(zoox.cm2_log[!is.na(zoox.cm2_log)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                 mean = mean(zoox.cm2_log, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(zoox.cm2_log, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
                 max = max(zoox.cm2_log, na.rm=TRUE) #calculate max, could also calculate min () if desired
)
FC_meanlog
...
Photosynthesis
```{r}       
GrossP_mean <- ddply(PR.df_B, c("Treatment", "Species", "Time.Point"), summarise, #summarize cell counts
                   N    = length(GrossP[!is.na(GrossP)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                   mean = mean(GrossP, na.rm=TRUE), #calculate mean of response variable, removing NA's
                   sd   = sd(GrossP, na.rm=TRUE), #calculate standard deviation
                   se   = sd / sqrt(N), #calculate standard error
                   max = max(GrossP, na.rm=TRUE) #calculate max, could also calculate min () if desired
)
GrossP_mean #display table

GrossP_meanCLADE <- ddply(PR.df_BPR.df_B.T0, c("Treatment", "Species", "Time.Point","Clade"), summarise, #summarize cell counts
                   N    = length(GrossP[!is.na(GrossP)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                   mean = mean(GrossP, na.rm=TRUE), #calculate mean of response variable, removing NA's
                   sd   = sd(GrossP, na.rm=TRUE), #calculate standard deviation
                   se   = sd / sqrt(N), #calculate standard error
...
GrossP_TFH_mean <- ddply(GrossP_TFH, c( "Species"), summarise, #summarize cell counts
                 N    = length(GrossP[!is.na(GrossP)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                 mean = mean(GrossP, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(GrossP, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
```{r}  
#Mcap 
GrossP_mean.Clade <- ddply(PR.df_B, c("Treatment", "Species", "Time.Point","Clade"), summarise, #summarize cell counts
                   N    = length(GrossP[!is.na(GrossP)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                   mean = mean(GrossP, na.rm=TRUE), #calculate mean of response variable, removing NA's
                   sd   = sd(GrossP, na.rm=TRUE), #calculate standard deviation
                   se   = sd / sqrt(N), #calculate standard error
...
PR.df_PR_B.T0_mean <- ddply(PR.df_BPR.df_B.T0, c( "Species"), summarise, #summarize cell counts
                 N    = length(RespPos[!is.na(RespPos)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                 mean = mean(RespPos, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(RespPos, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
                 max = max(RespPos, na.rm=TRUE) #calculate max, could also calculate min () if desired
)
PR.df_PR_B.T0_mean 
...
RespPos_mean <- ddply(PR.df_B, c("Treatment", "Species", "Time.Point"), summarise, #summarize cell counts
                   N    = length(RespPos[!is.na(RespPos)]), #calculate the length of the data frame, excluding NA’s *this part is important*
...
RespPos_TFH_mean <- ddply(RespPos_TFH, c( "Species"), summarise, #summarize cell counts
                 N    = length(RespPos[!is.na(RespPos)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                 mean = mean(RespPos, na.rm=TRUE), #calculate mean of RespPosonse variable, removing NA's
                 sd   = sd(RespPos, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
RespPos_mean.Clade <- ddply(PR.df_B, c("Treatment", "Species", "Time.Point","Clade"), summarise, #summarize cell counts
                   N    = length(RespPos[!is.na(RespPos)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                   mean = mean(RespPos, na.rm=TRUE), #calculate mean of RespPosonse variable, removing NA's
                   sd   = sd(RespPos, na.rm=TRUE), #calculate standard deviation
                   se   = sd / sqrt(N), #calculate standard error
...
#t0 only
PR.df_PR_B.T0_mean <- ddply(PR.df_BPR.df_B.T0, c( "Species"), summarise, #summarize cell counts
                 N    = length(PR_real[!is.na(PR_real)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                 mean = mean(PR_real, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(PR_real, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
                 max = max(PR_real, na.rm=TRUE) #calculate max, could also calculate min () if desired
)
PR.df_PR_B.T0_mean 
