...
#T1 only
    set.seed(30)
      Biomass_Pcomp_model.T1<-lmer(Yield_cu~Treatment+(1|Parent.ID)+(1|Tank.num), PAMdata2_stats_Pcomp.T1)
  anova(Biomass_Pcomp_model.T1) 
  ...
  #TF only
  Biomass_Pcomp_model.TF<-lmer(Yield_cu~Treatment+(1|Parent.ID)+(1|Tank.num), PAMdata2_stats_Pcomp.TF)
  anova(Biomass_Pcomp_model.TF) 
...
Yield_cu_Pvar_model<-lmer(Yield_cu~Days*Treatment*Clade+(1|Parent.ID:Frag.ID)+(1|Tank.num), PAMdata2_stats_Pvar)
  ...
  #T1 only 
  Biomass_Pvar_model.T1<-lmer(Yield_cu~Treatment+(1|Parent.ID)+(1|Tank.num), PAMdata2_stats_Pvar.T1)
  anova(Biomass_Pvar_model.T1) 
  ...
  #TF only
    Biomass_Pvar_model.TF<-lmer(Yield_cu~Treatment+(1|Parent.ID)+(1|Tank.num), PAMdata2_stats_Pvar.TF)
  anova(Biomass_Pvar_model.TF) 
...
Yield_cu_Pacu_model<-lmer(Yield_cu~Days*Treatment*Clade+(1|Parent.ID:Frag.ID)+(1|Tank.num), PAMdata2_stats_Pacu)
  ...
  #T1 only 
      Biomass_Pacu_model.T1<-lmer(Yield_cu~Treatment+(1|Parent.ID)+(1|Tank.num), PAMdata2_stats_Pacu.T1)
  anova(Biomass_Pacu_model.T1) 
  ...
  #TF only
  Biomass_Pacu_model.TF<-lmer(Yield_cu~Treatment+(1|Parent.ID)+(1|Tank.num), PAMdata2_stats_Pacu.TF)
  anova(Biomass_Pacu_model.TF) 
...
PAMdata2_mean.mcapclade <- ddply(PAMdata2_stats_Mcap, c("Treatment", "Clade", "Days"), summarise, #summarize cell counts
                   N    = length(Yield[!is.na(Yield)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                   mean = mean(Yield, na.rm=TRUE), #calculate mean of response variable, removing NA's
                   sd   = sd(Yield, na.rm=TRUE), #calculate standard deviation
                   se   = sd / sqrt(N), #calculate standard error
...
PAMdata2_mean.mcapclade #display table

PAMdata2_mean.mcapclade$Code<-paste(PAMdata2_mean.mcapclade$Clade, PAMdata2_mean.mcapclade$Treatment)
PAMdata2_mean.mcapclade$Days<-as.numeric(as.character(PAMdata2_mean.mcapclade$Days))

Mcap_PAM_plot<-ggplot(data=PAMdata2_mean.mcapclade, aes(x=Days, y=mean, group = Code, color=Code)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
...
PAM_psy<- PAM_psy[,c("Code","PAM.Yield_cu")] # keep only columns you need
...
PR_psy<-PR.df_B
 ...
 PRPR_psy<-merge(Photo_psy,Resp_psy, all = T)   
     PRPR_psy<-merge(PRPR_psy,P_R_psy2, all = T)   
...
#Remove Time 0
pData.T1TF<-subset(PhysioData, Time.Point=="T1"|Time.Point=="TF")

pData.T1TF<-na.omit(pData.T1TF) #this removes 9 samples (partent/time/species)

#deal with negatives by adding a constant (only P, R and P:R)
pData.T1TF$Photosynthesis<-pData.T1TF$Photosynthesis+1
pData.T1TF$Respiration<-pData.T1TF$Respiration+1
pData.T1TF$P_R<-pData.T1TF$P_R+1
