...
##next steps FOR OTU/DIV data
```{r}

###Checking which taxa are driving the significance####

##Using the mvabund package

dat.mv <- mvabund(ac_or[,2:18]) # makes an mvabund object with the bacterial data (matrix of RAW COUNTS)
Fac1 <- ac_or[,18:32] # makes a separate data sheet with all the metadata
# We will use RAW COUNTS but we still need to account for differences in sequencing depth
# We will follow the approach proposed by McMurdie & Holmes 2014 (PLOS Comput. Biol.), which is to
# include the total number of counts per sample explicitly in the model

# Lets add a column to Fac1 (the spreadsheet with covariates) of sums of counts per sample 
Fac1$Counts = rowSums(ac_or[,2:18])

# Have a look at the data by plotting 
plot(dat.mv ~ Fac1$MonthYear, cex.axis=0.55) # Raw data plot
plot(dat.mv ~ Fac1$MonthYear, type="bx", cex.axis=0.55, legend=F) # Box plot


# check the mean-variance relationship
meanvar.plot(dat.mv ~ Fac1$MonthYear, pch=19)
# mean increases with the variance - useful to use a negative binomial distribution


# Lets fit the model
dat.nb = manyglm(dat.mv ~ Counts + MonthYear, family="negative.binomial", data=Fac1)
#The order you put your variables in matters because R uses Type 1 Sums of Squares, so first 
#you want to take into account the bias in counts, followed by looking at differences in species
#what you really care about should go last
#Family = type of data distribution (e.g. poisson, normal, binomial) but for sequencing data
#it is standard to use negative binomial

# Note: you could also fit Counts as an 'offset', which assumes a proportional increase
# it should be 'log' of the Counts as the link function for neg.binom. is log
# dat.nb = manyglm(dat.mv ~ Species + offset(log(Counts)), family="negative.binomial", data=Fac1)


# Check the model fit by plotting the residuals
...
# The model is fit so now we need to run the statistical test
dat.aov = anova(dat.nb, nBoot=999, p.uni="adjusted") # uses bootstraps-can take a long time to run
#We can use nBoot = 100 for trial, but should use 999 for publication

#if you want to save this as an .RData file so you don't have to run it again use:
save(dat.aov, file = "/Users/jc224701/Desktop/chg_all/R_output/16S/Chapter 2/dat_mvabund_cl.RData")

# Get the output of the test
dat.aov$table # multivariate table
#Now that we have taken account of the sequencing bias using counts, then it doesn't matter if counts are 
#significant, we just care about species being significant

##Ouput:
#Res.Df Df.diff      Dev Pr(>Dev)
#(Intercept)    101      NA       NA       NA
#Counts         100       1 678.9981    0.001
#MonthYear       91       9 130.2149    0.479

# Obviuosly there is no difference in the multivariate test statistic for my dataset.  
# BUT if we were to see a difference, we would then ask which variables constituted this effect.

# We can look at test statistics and p values: 
...
# We can use code to ask: Are any of these significantly different? (i.e. p<0.05)
...
#Then ask: Which are the significant ones?
...
plot(dat.mv ~ Fac1$Species, var.subset=diffs, type="bx", cex.axis=0.55)
plot(dat.mv ~ Fac1$Species, var.subset=diffs, type="bx", cex.axis=0.55, legend=F)
