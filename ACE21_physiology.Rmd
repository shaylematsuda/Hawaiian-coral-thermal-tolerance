...
CellCounts_psy<- FC_stats_T1TF[,c("Frag.ID","Parent.ID" ,"Species","Treatment","Tank.num","Time.Point","zoox.cm2_log")] # keep only columns you need
...
Chla_psy<- Chla_stats_T1TF[,c("Parent.ID" ,"Species","Treatment","Time.Point","ChlaSA_quart")] # keep only columns you need
...
Protein_psy<- Prot_stats_T1TF[,c("Parent.ID" ,"Species","Treatment","Time.Point","mg.mm2_sqrt")] # keep only columns you need
...
PR_psy<-PR.df_B
  ...
  #photo
  Photo_psy<- PR_psy[,c("Frag.ID","Parent.ID" ,"Species","Treatment","Tank.num","Time.Point","Photo","Code")] 
    Photo_psy<-Photo_psy %>% group_by(Code) %>% mutate(mean=mean(Photo)) # mean by code, this worked
    names(Photo_psy)[names(Photo_psy) == "mean"] <- "Photosynthesis" #change name
    Photo_psy<-Photo_psy[,c("Code","Photosynthesis")] #remove col
    Photo_psy<-unique(Photo_psy) #remove duplicates, now you have 
  
   #Resp
  Resp_psy<- PR_psy[,c("Frag.ID","Parent.ID" ,"Species","Treatment","Tank.num","Time.Point","Resp","Code")] 
   ...
   #PR
  PR_psy<- PR_psy[,c("Frag.ID","Parent.ID" ,"Species","Treatment","Tank.num","Time.Point","PR","Code")] 
    PR_psy<-PR_psy %>% group_by(Code) %>% mutate(mean=mean(PR)) # mean by code, this worked
    names(PR_psy)[names(PR_psy) == "mean"] <- "PR" #change name
    PR_psy<-PR_psy[,c("Code","PR")] #remove col
    PR_psy<-unique(PR_psy) #remove duplicates, now you have 
    
 PRPR_phy<-merge(Photo_psy,Resp_psy, all = T)   
     PRPR_phy<-merge(PRPR_phy,PR_psy, all = T)   
