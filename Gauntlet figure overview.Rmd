...
# compare ITS to 16s at T0
  ...
  # 16s changes: rename sample S208b = S208
    ...
    #pull TAX table
     ...
     #pull phy tree table
     phyTree.16sT0 = phy_tree(Bac.s.rel.u.T0) #pull out tree
...
# compare ITS to 16s at T1
 ...
 #rename
   #change names 16s S1857b to S1857 and SN134b to SN134
    ...
    #pull TAX table
     OTU.16ST1.Ambient.tax = as(tax_table(Bac.T1.Ambient.mantel), "matrix") #pull tax

#pull phy tree table
     tree.seq.T1Amb = phy_tree(Bac.T1.Ambient.mantel) #pull out tree
    ...
    #remake the phyloseq obj
      samdata.Ambient = sample_data(aSD.16ST1.Ambient)
        ...
        taxtab.Ambient = tax_table(OTU.16ST1.Ambient.tax)
        phy_tree.amb = read_tree(tree.seq.T1Amb)

BacT1.Ambient_mantel = phyloseq(otu_table(seqtab.Ambient), tax_table(taxtab.Ambient), sample_data(samdata.Ambient), phy_tree(phy_tree.amb))
...
#both 37 samples
        ...
        Bac.T1.Ambient.mantel <- BacT1.Ambient_mantel %>% ps_arrange(sample_name.1, desc(sample_name.1))
...
# compare ITS to 16s at TF
  ...
  # 16s changes: remove  "S2893"   "S3481"  "S775" 
   ...
   #rename S3476b to S3476
    ...
    #pull TAX table
     OTU.16STF.High.tax = as(tax_table(Bac.TF.High.mantel), "matrix") #pull tax
 #pull phy tree table
     tree.seq.T1High = phy_tree(Bac.TF.High.mantel) #pull out tree
    ...
    #remake the phyloseq obj
      samdata.High = sample_data(aSD.16STF.High)
        ...
        taxtab.High = tax_table(OTU.16STF.High.tax)
        phy_tree = read_tree(tree.seq.T1High)

          BacTF.High_mantel = phyloseq(otu_table(seqtab.High), tax_table(taxtab.High), sample_data(samdata.High), phy_tree(phy_tree))
   ...
   #ITS2 - remove samps not in the 16s df - "S3474" "S3471"  "SN138"   "S2018"  "S840"   "S3515"  "S477"   "S3489"  "SN141"  "S1771"  "S1166b"
    ...
    #remake the phyloseq obj
      samdata.High = sample_data(aSD.ITS2TF.High)
        ...
        Bac.TF.High.mantel <- BacTF.High_mantel %>% ps_arrange(sample_name.1, desc(sample_name.1))
...
#set up physio T1
  ...
  #save just sample names
      ...
      # compare physio to 16s at T1
    #samples in 16s but not phys: RENAME: S1443b, SN108b, SN197b, REMOVE: SN161, SN181
      #samples in phys but not 16s: REMOVE: S1195, S1418, S1573, S1895, S2424, S3035, S785, SN126, SN209 
...
#rename 16S:S1443b, SN108b, SN197b,
    ...
    #pull SAMPLE DATA
     ...
     aSD.16sT1.Amb<-as.data.frame(aSD.16sT1.Amb) #change to df
        row.names(aSD.16sT1.Amb)[3] <- "S1443"  #replace name 
        row.names(aSD.16sT1.Amb)[20] <- "SN108"  #replace name 
        row.names(aSD.16sT1.Amb)[28] <- "SN197"  #replace name 
  ...
  # reorder ps object so samples are in the same order
        rownames(physio_T1.Amb_mantel) <- physio_T1.Amb_mantel[,1] #make first col row names
         ...
         vec <- c("S1144","S1254","S1443","S153","S199","S2286","S249","S284","S419","S431","S436","S554","S650","S664","S761","S797","S809","S848","SN104","SN108","SN117","SN124","SN149","SN170","SN172","SN184","SN192","SN197","SN198","SN222","SN37","SN45","SN49","SN78","SN89","SN92")               # Create  vector order
         physio_T1.Amb_mantel.ord <- physio_T1.Amb_mantel[match(vec, physio_T1.Amb_mantel$ID), ]  
...
#build matrix  
  ...
  #physio T1 Amb
       physio_T1.Amb_mantel2<-  physio_T1.Amb_mantel.ord[,3:10]
...
#### T1 Mcap only ####        
 # Bac_T1_mantel physio_T1_mantel
    ...
    physio_T1.Amb_mantel.Mcap  <- subset(physio_T1.Amb_mantel.ord, species == "Montipora_capitata")
...
#### T1 Pcomp only ####        
 # Bac_T1_mantel physio_T1_mantel
    ...
    physio_T1.Amb_mantel.Pcomp  <- subset(physio_T1.Amb_mantel.ord, species == "Porites_compressa")
 ...
 # Bac_T1.Amb_mantel physio_T1.Amb_mantel
    ...
    physio_T1.Amb_mantel.Pvar  <- subset(physio_T1.Amb_mantel.ord, species == "Pavona_varians")
 ...
 # Bac_T1.Amb_mantel physio_T1.Amb_mantel
    ...
    physio_T1.Amb_mantel.Pacu  <- subset(physio_T1.Amb_mantel.ord, species == "Pocillopora_acuta")
...
#rename 16S:S3476b
  ...
  # reorder ps object so samples are in the same order
        rownames(physio_TF.High_mantel) <- physio_TF.High_mantel[,1] #make first col row names
         ...
         vec <- c("S1166","S1413","S1437","S1526","S1803","S2136","S2328","S2663","S2731","S2879","S2893","S2948","S3204","S335","S3383","S3422","S3444","S3467","S3470","S3473","S3476","S3480","S3486","S3487","S3491","S3517","S473","S775","SN55")               # Create  vector order
         
         physio_TF.High_mantel.ord <- physio_TF.High_mantel[match(vec, physio_TF.High_mantel$ID), ]  
#build matrix  
  #16s TF        
  ...
  #physio TF high
        physio_TF.High_mantel2<-  physio_TF.High_mantel.ord[,3:10]
...
#16 remove: S1226, S1688, S2214, S2226, S377 S471 S955 SN110 SN158, SN169, SN217, SN42 S785b
  ...
  # reorder ps object so samples are in the same order
                ...
                rownames(physio_TF.Ambient_mantel) <- physio_TF.Ambient_mantel[,1] #make first col row names
                 ...
                 vec <- c("S1037","S1061","S1104","S3081","S3164","S3337","S3442","S403","S420","S422","S579","S832","SN105","SN115","SN122","SN131","SN132","SN152","SN189","SN191","SN205","SN211","SN77","SN96","SN98")               # Create  vector order
...
### Try this again with the physio data set culled down to fewer metrics (goal of having more samples included. )
...
library("factoextra")
library(phyloseq)
apriori<-read.csv("apriori3.csv") #ths only loads bc you opened in textwrangler and hit a hard return and saved....
...
apriori.df<-apriori[2:13]
summary(apriori.df)[,1:4]
  #apriori.df$clade<-as.factor(apriori.df$clade)
    apriori.df$Symb.genera<-as.factor(apriori.df$Symb.genera)
  apriori.df$Symb.fidelity<-as.factor(apriori.df$Symb.fidelity)
  ...
  apriori.df$size<-as.factor(apriori.df$size)
    apriori.df$calcification<-as.factor(apriori.df$calcification)
    apriori.df$tissue.thick<-as.factor(apriori.df$tissue.thick)
    apriori.df$Branching<-as.factor(apriori.df$Branching)
        apriori.df$sex.ord<-as.factor(apriori.df$sex.ord)
        apriori.df$color<-as.factor(apriori.df$color)
        apriori.df$bleaching<-as.factor(apriori.df$bleaching)
        apriori.df$transmission<-as.factor(apriori.df$transmission)

ggrepel.max.overlaps = Inf
res.apriori<-MCA(apriori.df,  graph = TRUE, ncp=10)
print(res.apriori)
...
# Physio T0 only new gauntlet fig
```{r}
library(vegan)

PhysioData<-read.csv("PhysioData.csv")
pData.T0<-subset(PhysioData, time.point=="T0")
pData.T0<-na.omit(pData.T0)

# pData.T0$photosynthesis<-pData.T0$photosynthesis+1
pData.T0$respiration<-pData.T0$respiration+20
# pData.T0$p.r<-pData.T0$p.r+1



phy.dist.T0 <- vegdist(pData.T0 [15:24], method="bray") #make dist matrix
phy.div.T0 <- adonis2(pData.T0 [15:24] ~ species, data = pData.T0 [1:7], permutations = 999, method="bray")
phy.div.T0 #view, 
