...
Do not run this section unless the phyloseq objects need to be rebuilt from the SymPortal outputs.
...
# Auditable sample reconciliation. recorded_id is the SymPortal/Frag.ID label;
# canonical_id documents the biological sample relationship without renaming data.
sample_reconciliation <- read.csv(
  "Data/ITS2_sample_reconciliation.csv",
  stringsAsFactors = FALSE
)
excluded_sample_ids <- unique(
  sample_reconciliation$recorded_id[sample_reconciliation$action == "exclude"]
)

filter_its2_samples <- function(ps, minimum_reads = NULL) {
  if (!is.null(minimum_reads)) {
    ps <- prune_samples(sample_sums(ps) >= minimum_reads, ps)
  }
  prune_samples(!(sample_names(ps) %in% excluded_sample_ids), ps)
}
...
# Phyloseq accepts a data.frame directly. Keeping it as a data.frame preserves
# numeric and factor types that would be coerced to character by as.matrix().
sam0 <- as.data.frame(Symp_sub_Meta)
rownames(sam0) <- sam0$sample_name
sam <- sample_data(sam0[, setdiff(names(sam0), "sample_name"), drop = FALSE])
...
# Apply the reconciliation table once to the profile object.
coral_profile2 <- filter_its2_samples(coral_profile)
...
# Apply the read threshold and reconciliation table to one consistently named
# non-rarefied object. This correctly excludes S2893 before rarefaction.
coral_DIV2_nonrare <- filter_its2_samples(coralDIV2, minimum_reads = 7000)
...
#ggsave("Figures/ITS2_T0_samp_leg.pdf", plot = T0_ITS2_samp, device = "pdf", width = 20, height = 6)


#Make T0 with DIVs ####
#try glom use raw counts 
DIVs_T0<-subset_samples(coral_DIV2_rare, Time.Point=="T0") #filtered rarefied counts
...
T0_ITS2_DIV_samp <- T0DIV.p + facet_wrap(~Species * Clade, scales = "free_x", nrow = 1)
T0_ITS2_DIV_samp

#ggsave("Figures/ITS2_T0_DIV_samp.pdf", plot = T0_ITS2_DIV_samp, device = "pdf", width = 20, height = 6)
...
ordination_scores_with_metadata <- function(ordination, ps) {
  scores <- as.data.frame(vegan::scores(ordination, display = "sites"))
  scores$recorded_id <- rownames(scores)
  metadata <- as(sample_data(ps), "data.frame")
  metadata$recorded_id <- rownames(metadata)

  stopifnot(!anyDuplicated(scores$recorded_id))
  stopifnot(!anyDuplicated(metadata$recorded_id))

  joined <- left_join(scores, metadata, by = "recorded_id")
  stopifnot(nrow(joined) == nrow(scores))
  joined
}

# Save the all-time-point object used by ACE21_16s_analysis.Rmd.
saveRDS(DIV_RelA_stats, "Data/ITS2_phyloseq.rds")

#bray curtis presence-absense and abundance
```

## PERMANOVA design inventory

`adonis2()` does not fit random effects. Repeated or grouped observations can instead be handled by restricting permutations with `strata` or a `permute::how()` design. The table below records the current analyses and likely blocking variables; it does not change the tests until the experimental unit is confirmed.

```{r}
permanova_design_inventory <- data.frame(
  scope = c(
    "T0 all species",
    "T1 all species",
    "TF all species",
  ...
  phyloseq_object = c(
    ...
    "DIV_RelA_stats.T1",
    "DIV_RelA_stats.TF",
    "species-specific T0 object",
    "species-specific T1 object",
    "species-specific TF object"
  ),
  metadata_frame = c(
    "samp.df.T0",
    "samp.df",
    "samp.df",
    "species-specific sample_data",
    "species-specific sample_data",
    "species-specific sample_data"
  ),
  current_formula = c(
    ...
    "Species * Treatment",
    "Clade",
    "Treatment; Clade * Treatment",
    "Treatment; Clade * Treatment"
  ),
  repeated_unit_to_review = c(
    "Tank.num",
    "Parent.ID and Tank.num",
    "Parent.ID and Tank.num",
    "Tank.num",
    "Parent.ID and Tank.num",
    "Parent.ID and Tank.num"
...
scores.ord.DIV_RelA_stats.T0 <- ordination_scores_with_metadata(ord.DIV_RelA_stats.T0, DIV_RelA_stats.T0)
...
scale_color_manual(values=c("goldenrod1","darkorange3", "pink1","firebrick", "palegreen2","darkgreen", "powderblue","#0072B2")) +  # Set colors for points
   scale_fill_manual(values=c("goldenrod1","darkorange3", "pink1","firebrick", "palegreen2","darkgreen", "powderblue","#0072B2"))+
...
#all speices
set.seed(30)
bc.all.T1 <- phyloseq::distance(DIV_RelA_stats.T1, method = "bray")#make bray curtis data matrix
  samp.df.T1 <- as(sample_data(DIV_RelA_stats.T1), "data.frame") #sample df
#adonis test
adonis2(bc.all.T1 ~ Species*Treatment, data = samp.df.T1)
    
# Homogeneity of dispersion test
betaAll.T1 <- betadisper(bc.all.T1, samp.df.T1$Species)
permutest(betaAll.T1, pairwise = T)

#Ordination NMDS for all species
set.seed(30)
ord.DIV_RelA_stats.T1 <- ordinate(DIV_RelA_stats.T1, "NMDS", "bray", trymax = 100) 
stressplot(ord.DIV_RelA_stats.T1)
scores.ord.DIV_RelA_statsT1 <- ordination_scores_with_metadata(ord.DIV_RelA_stats.T1, DIV_RelA_stats.T1)
...
set.seed(30)
#make bray curtis data matrix
bc.all.TF <- phyloseq::distance(DIV_RelA_stats.TF, method = "bray")
  samp.df.TF <- as(sample_data(DIV_RelA_stats.TF), "data.frame") #sample df
#adonis test
adonis2(bc.all.TF ~ Species*Treatment, data = samp.df.TF)
    
# Homogeneity of dispersion test
betaAll.TF <- betadisper(bc.all.TF, samp.df.TF$Species)
permutest(betaAll.TF, pairwise = T)

#Ordination NMDS for all species
set.seed(30)
ord.DIV_RelA_stats.TF <- ordinate(DIV_RelA_stats.TF, "NMDS", "bray", trymax = 100) 
stressplot(ord.DIV_RelA_stats.TF)
scores.ord.DIV_RelA_statsTF <- ordination_scores_with_metadata(ord.DIV_RelA_stats.TF, DIV_RelA_stats.TF)
scores.ord.DIV_RelA_statsTF$Code3<-paste(scores.ord.DIV_RelA_statsTF$Species,scores.ord.DIV_RelA_statsTF$Treatment)

ggplot(scores.ord.DIV_RelA_statsTF, aes(x = NMDS1, y = NMDS2, color = Species, linetype=Treatment)) +
  geom_point(aes(shape=Treatment), size=4) +
  stat_ellipse(geom = "polygon",
               aes(fill = Code3), 
               alpha = 0.25)+
...
#T1 and TF
set.seed(30)
ord.mcap <- ordinate(mcap.RelRare.T1F, method="NMDS", distance="bray", trymax = 100)
stressplot(ord.mcap)
scores.mcap.samp.df <- ordination_scores_with_metadata(ord.mcap, mcap.RelRare.T1F)
...
#T1 and TF
set.seed(30)
ord.pcomp <- ordinate(pcomp.RelRare.T1F, method="NMDS", distance="bray", trymax = 100)
stressplot(ord.pcomp)
scores.pcomp.samp.df <- ordination_scores_with_metadata(ord.pcomp, pcomp.RelRare.T1F)
scores.pcomp.samp.df$Code <- paste(scores.pcomp.samp.df$Parent.ID, scores.pcomp.samp.df$Time.Point)
scores.pcomp.samp.df$Code2 <- paste(scores.pcomp.samp.df$Treatment, scores.pcomp.samp.df$Time.Point)
...
#ggsave("Figures/PcompT1its2.pdf")
...
#T1 and TF
set.seed(30)
ord.pvar <- ordinate(pvar.RelRare.T1F, method="NMDS", distance="bray", trymax = 100)
stressplot(ord.pvar)
scores.pvar.samp.df <- ordination_scores_with_metadata(ord.pvar, pvar.RelRare.T1F)
scores.pvar.samp.df$Code <- paste(scores.pvar.samp.df$Parent.ID, scores.pvar.samp.df$Time.Point)
scores.pvar.samp.df$Code2 <- paste(scores.pvar.samp.df$Treatment, scores.pvar.samp.df$Time.Point)
...
#T1 and TF
set.seed(30)
ord.pacu <- ordinate(pacu.RelRare.T1F, method="NMDS", distance="bray", trymax = 100)
stressplot(ord.pacu)
scores.pacu.samp.df <- ordination_scores_with_metadata(ord.pacu, pacu.RelRare.T1F)
scores.pacu.samp.df$Code <- paste(scores.pacu.samp.df$Parent.ID, scores.pacu.samp.df$Time.Point)
scores.pacu.samp.df$Code2 <- paste(scores.pacu.samp.df$Treatment, scores.pacu.samp.df$Time.Point)
...
```{r}
ITS2_lineage_NMDS_panel <- arrangeGrob(McapITS2_T1TF, PcompITS2_T1TF, PvarITS2_T1TF, PacuITS2_T1TF, nrow = 1)
ggsave("Figures/ITS2_T1_TF_NMDS.pdf", plot = ITS2_lineage_NMDS_panel, width = 16, height = 7, units = "in")

```
All plots: Figure S15
```{r}
ITS2_treatment_NMDS_panel <- arrangeGrob(McapITS2_T1TFplot, PcompITS2_T1TFplot, PvarITS2_T1TFplot, PacuITS2_T1TFplot, nrow = 4)
ggsave("Figures/ITS2_T1_TF_NMDS_FigS15.pdf", plot = ITS2_treatment_NMDS_panel, width = 7, height = 16, units = "in")

# All time points: visual summary for Figure S16
DIV_RelA_stats.all <- prune_taxa(
  taxa_sums(DIV_RelA_stats) > 0,
  DIV_RelA_stats
)

set.seed(30)
ord.DIV_RelA_stats.all <- ordinate(
  DIV_RelA_stats.all,
  method = "NMDS",
  distance = "bray",
  trymax = 100
)
stressplot(ord.DIV_RelA_stats.all)

scores.ord.DIV_RelA_stats.all <- ordination_scores_with_metadata(
  ord.DIV_RelA_stats.all,
  DIV_RelA_stats.all
)
scores.ord.DIV_RelA_stats.all$Time_Treatment <- interaction(
  scores.ord.DIV_RelA_stats.all$Time.Point,
  scores.ord.DIV_RelA_stats.all$Treatment,
  sep = "_"
)

ITS2_all_time_NMDS <- ggplot(
  scores.ord.DIV_RelA_stats.all,
  aes(x = NMDS1, y = NMDS2, colour = Species, shape = Time_Treatment)
) +
  geom_point(size = 4) +
...
ITS2_all_time_NMDS
ggsave("Figures/ITS2_all_time_NMDS_FigS16.pdf", plot = ITS2_all_time_NMDS)
...
Sp.colors <- c(species_colors, "High" = "darkRed", "Ambient" = "navyBlue")
...
alpha_values <- microbiome::alpha(its2.al.alpha2, index = "all")
alpha_values$recorded_id <- rownames(alpha_values)

# Join metadata by the phyloseq sample name instead of assuming row order.
alpha_metadata <- as(sample_data(its2.al.alpha2), "data.frame")
alpha_metadata$recorded_id <- rownames(alpha_metadata)

stopifnot(!anyDuplicated(alpha_values$recorded_id))
stopifnot(!anyDuplicated(alpha_metadata$recorded_id))

erich.tab2its <- alpha_values %>%
  dplyr::select(recorded_id, observed, evenness_pielou) %>%
  left_join(
    alpha_metadata %>%
      dplyr::select(recorded_id, ID, Treatment, Time.Point, Parent.ID, Species, Clade, Tank.num),
    by = "recorded_id"
  )

stopifnot(nrow(erich.tab2its) == nrow(alpha_values))
stopifnot(!anyNA(erich.tab2its$Species))

# recorded_id is the SymPortal sample_name/Frag.ID. ID is retained separately
# because five metadata records differ and must not be silently canonicalized.
sample_id_comparison <- erich.tab2its %>%
  distinct(recorded_id, ID) %>%
  mutate(
    same_id = recorded_id == ID,
    duplicate_suffix = grepl("[bB]$", recorded_id)
  )
sample_id_comparison %>% filter(!same_id)

erich.tab2its$Time.Point <- factor(erich.tab2its$Time.Point, levels = c("T0", "T1", "TF"))
```

# Refactored alpha-diversity models and figures

The models below cover observed richness and Pielou evenness using the same design for every species. T0 models use tank as the grouping term. T1/TF models use parent identity because fragments from a parent are measured repeatedly across treatment and time. Model failures and singular fits are retained in the result objects rather than being hidden.

```{r}
alpha_species_tables <- list(
  mcap = subset(erich.tab2its, Species == "Montipora_capitata"),
  pcomp = subset(erich.tab2its, Species == "Porites_compressa"),
  pvar = subset(erich.tab2its, Species == "Pavona_varians"),
  pacu = subset(erich.tab2its, Species == "Pocillopora_acuta")
)

# Compatibility names for older downstream code.
erich.tab2its.mcap <- alpha_species_tables$mcap
erich.tab2its.pcomp <- alpha_species_tables$pcomp
erich.tab2its.pvar <- alpha_species_tables$pvar
erich.tab2its.pacu <- alpha_species_tables$pacu

fit_alpha_model <- function(data, response, fixed_effects, emmeans_specs, random_effect = NULL) {
  analysis_data <- droplevels(data)
  formula_terms <- if (is.null(random_effect)) fixed_effects else paste(fixed_effects, "+", random_effect)
  model_formula <- as.formula(paste(response, "~", formula_terms))

  tryCatch({
    model <- if (is.null(random_effect)) {
      lm(model_formula, data = analysis_data)
    } else {
      lmer(model_formula, data = analysis_data, REML = TRUE)
    }
    estimated_means <- emmeans(model, specs = as.formula(emmeans_specs))

    list(
      formula = model_formula,
      n = nrow(analysis_data),
      model_type = if (is.null(random_effect)) "lm" else "lmer",
      model = model,
      anova = anova(model),
      emmeans = estimated_means,
      contrasts = pairs(estimated_means, adjust = "Tukey"),
      singular = if (inherits(model, "merMod")) lme4::isSingular(model) else FALSE,
      error = NULL
    )
  }, error = function(e) {
    list(
...
alpha_model_results <- setNames(vector("list", length(alpha_metrics)), names(alpha_metrics))

for (metric_name in names(alpha_metrics)) {
  response <- alpha_metrics[[metric_name]]
  metric_results <- list()

  # T0 comparison across host species.
  t0_all <- subset(erich.tab2its, Time.Point == "T0")
  metric_results$T0_all_species <- fit_alpha_model(
    t0_all,
    response,
    fixed_effects = "Species",
    emmeans_specs = "~ Species",
    random_effect = "(1 | Tank.num)"
  )

  for (species_code in names(alpha_species_tables)) {
    species_data <- alpha_species_tables[[species_code]]

    # T0 lineage comparison. Montipora lineages are C31 and C31D4.
    t0_data <- subset(species_data, Time.Point == "T0")
    metric_results[[paste(species_code, "T0", sep = "_")]] <- fit_alpha_model(
      t0_data,
      response,
      fixed_effects = "Clade",
      emmeans_specs = "~ Clade",
    ...
    # Combined T1/TF model.
    post_t0_data <- subset(species_data, Time.Point %in% c("T1", "TF"))
    post_t0_data$Time.Point <- droplevels(post_t0_data$Time.Point)
    post_t0_fixed <- if (dplyr::n_distinct(post_t0_data$Clade, na.rm = TRUE) > 1) {
      "Time.Point * Treatment * Clade"
    } else {
      "Time.Point * Treatment"
    }
    post_t0_emmeans <- if (grepl("Clade", post_t0_fixed)) {
      "~ Time.Point * Treatment * Clade"
    } else {
      "~ Time.Point * Treatment"
    }
    metric_results[[paste(species_code, "T1TF", sep = "_")]] <- fit_alpha_model(
      post_t0_data,
      response,
      fixed_effects = post_t0_fixed,
      emmeans_specs = post_t0_emmeans,
      random_effect = "(1 | Parent.ID)"
    )

    # Separate T1 and TF treatment models.
    for (time_point in c("T1", "TF")) {
      time_data <- subset(species_data, Time.Point == time_point)
      time_fixed <- if (dplyr::n_distinct(time_data$Clade, na.rm = TRUE) > 1) {
        "Treatment * Clade"
      } else {
        "Treatment"
      }
      time_emmeans <- if (grepl("Clade", time_fixed)) {
        "~ Treatment * Clade"
      } else {
        "~ Treatment"
      }
      metric_results[[paste(species_code, time_point, sep = "_")]] <- fit_alpha_model(
        time_data,
        response,
        fixed_effects = time_fixed,
        emmeans_specs = time_emmeans,
        random_effect = "(1 | Parent.ID)"
      )
    }
  }

  alpha_model_results[[metric_name]] <- metric_results
}

alpha_model_status <- do.call(rbind, lapply(names(alpha_model_results), function(metric_name) {
  do.call(rbind, lapply(names(alpha_model_results[[metric_name]]), function(scope) {
    result <- alpha_model_results[[metric_name]][[scope]]
    data.frame(
      metric = metric_name,
      scope = scope,
      formula = paste(deparse(result$formula), collapse = " "),
      n = result$n,
...
rownames(alpha_model_status) <- NULL
alpha_model_status

summarize_alpha_metric <- function(data, response) {
  data %>%
    group_by(Species, Treatment, Time.Point, Clade) %>%
    summarise(
...
make_alpha_plot <- function(summary_data, y_label, include_clade = FALSE) {
  grouping <- if (include_clade) interaction(summary_data$Treatment, summary_data$Clade) else summary_data$Treatment
  summary_data$plot_group <- grouping

  ggplot(summary_data, aes(x = Time.Point, y = mean, group = plot_group, color = Treatment)) +
    geom_errorbar(aes(ymin = mean - se, ymax = mean + se), width = 0.1) +
    geom_point(aes(shape = if (include_clade) Clade else Treatment), size = 3) +
    geom_line(aes(linetype = if (include_clade) Clade else Treatment)) +
    facet_wrap(~Species) +
    scale_color_manual(values = c("Ambient" = "dodgerblue3", "High" = "darkred")) +
    labs(x = NULL, y = y_label, color = "Treatment", shape = "Clade", linetype = "Clade") +
    theme_classic()
}

alpha_richness_summary <- summarize_alpha_metric(erich.tab2its, "observed")
alpha_evenness_summary <- summarize_alpha_metric(erich.tab2its, "evenness_pielou")

ITS2_richness_summary_plot <- make_alpha_plot(
  alpha_richness_summary,
  "Observed richness"
)
ITS2_richness_clade_plot <- make_alpha_plot(
  alpha_richness_summary,
  "Observed richness",
  include_clade = TRUE
)
ITS2_evenness_summary_plot <- make_alpha_plot(
  alpha_evenness_summary,
  "Pielou evenness"
)
ITS2_evenness_clade_plot <- make_alpha_plot(
  alpha_evenness_summary,
  "Pielou evenness",
  include_clade = TRUE
)

ggsave("Figures/ITS2_alpha_richness_summary.pdf", ITS2_richness_summary_plot, width = 12, height = 7)
ggsave("Figures/ITS2_alpha_richness_by_clade.pdf", ITS2_richness_clade_plot, width = 12, height = 7)
ggsave("Figures/ITS2_alpha_evenness_summary.pdf", ITS2_evenness_summary_plot, width = 12, height = 7)
ggsave("Figures/ITS2_alpha_evenness_by_clade.pdf", ITS2_evenness_clade_plot, width = 12, height = 7)
```

# Legacy alpha-diversity workflow (reference only)

The original repeated model and plotting blocks are retained temporarily for comparison but are not evaluated. The refactored section above replaces them with consistently named results for both metrics and every species.
...
erich.tab2its.T0$Species <- factor(
  erich.tab2its.T0$Species,
  levels = names(species_colors)
)

ITS2_rich <- ggplot(erich.tab2its.T0, aes(x = Species, y = observed, fill = Species)) +
  geom_boxplot() +
  scale_fill_manual(values = species_colors) +
  labs(x = "Host Species", y = "Microbial Richness", fill = "Species") +
  theme_classic() +
...
ITS2_even <- ggplot(
  erich.tab2its.T0,
  aes(x = Species, y = evenness_pielou, fill = Species)
) +
  geom_boxplot() +
  scale_fill_manual(values = species_colors) +
  labs(x = "Host Species", y = "Microbial Evenness", fill = "Species") +
  theme_classic() +
  theme(axis.text.x = element_blank())
ITS2_even
