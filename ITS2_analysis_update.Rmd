...
T0.p+facet_wrap(~Species,scales = "free_x") #plot by sample 
...
#FIGURE 2B ITS2 T0 - by speices only 
# Define your colors
...
its2plot<-ggplot(scores.ord.DIV_RelA_stats.T0, aes(x = NMDS1, y = NMDS2, color = Species)) + 
  geom_point(size = 2) +
    stat_ellipse(aes(fill = Species), level = 0.95, alpha = 0.2, linetype = 1, size = 0.5, geom = "polygon") + 
...
ggsave("Figures/NMDS_ITS2_T0species.pdf", plot = its2plot, device = "pdf", width = 4, height = 3)

#FIGURE 2B ITS2 T0 - shown by Profile group  

its2plot<-ggplot(scores.ord.DIV_RelA_stats.T0, aes(x = NMDS1, y = NMDS2, color = Code)) + 
  geom_point(size = 2) +
 #geom_mark_ellipse(aes(fill = Code), alpha = 0.2, linetype = 1, size=.5) + # Use geom_mark_ellipse for both fill and border
...
ggsave("Figures/NMDS_ITS2_Tclade.pdf", plot = its2plot, device = "pdf", width = 4, height = 3)
...
#adonis test
adonis2(bc.all.Tall ~ Species, data = samp.df.Tall)
...
ggplot(scores.ord.DIV_RelA_stats.Tall, aes(x = NMDS1, y = NMDS2)) + 
  ...
  ggtitle("NMDS its2 Bray Curtis all clade") +
  theme_classic()
#ggsave("bc_all.pdf")
