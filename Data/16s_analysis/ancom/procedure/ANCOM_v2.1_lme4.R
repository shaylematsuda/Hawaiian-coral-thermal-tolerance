##ANCOM functions##
##From Frederick Huang Lin: https://github.com/FrederickHuangLin##
library(lme4)
library(nlme)
library(dplyr)
library(ggplot2)
...
ANCOM = function(feature_table, meta_data, struc_zero = NULL, main_var, p_adj_method = "BH", 
  ...
  if (is.null(rand_formula) & is.null(adj_formula)) {
    ...
    if (is.null(adj_formula)) {
      ...
      tformula = formula(paste("x ~", main_var, "+", rand_formula, sep = " "))
    }else {
      # Random coefficients/slope model
      tformula = formula(paste("x ~", main_var, "+", adj_formula, "+", rand_formula, sep = " "))
    }
  }
